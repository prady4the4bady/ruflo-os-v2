# Prady Research Widget Integration Guide

## 📦 Overview

The Prady Research Widget is a standalone, embeddable component that brings Goal-Oriented Action Planning research capabilities to any website. It's fully self-contained with no external dependencies required on the host page.
...
<script>
  window.addEventListener('load', function() {
    if (window.KryosResearchWidget) {
      window.KryosResearchWidget.init('my-custom-container');
    }
  });
</script>
```
...
window.addEventListener('DOMContentLoaded', function() {
  if (window.KryosResearchWidget) {
    window.KryosResearchWidget.init('kryos-research-widget-container');
  }
});
```
...
```javascript
console.log(window.KryosResearchWidget.version); // "1.0.0"
```
...
<script>
  window.addEventListener('load', function() {
    window.KryosResearchWidget.init('widget-1');
    window.KryosResearchWidget.init('widget-2');
  });
</script>
```
...
export default function KryosResearchWidget() {
  useEffect(() => {
    // Load widget script
  ...
  return <div id="kryos-research-widget-container" />;
}
```
