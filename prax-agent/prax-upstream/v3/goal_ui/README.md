...
> Goal-Oriented Action Planning UI for autonomous AI research workflows. Part of the [Prady](https://github.com/ruvnet/kryos) platform.
> Live: [goal.ruv.io](https://goal.ruv.io/) · Agents dashboard: [goal.ruv.io/agents](https://goal.ruv.io/agents)

Turn plain-English research goals into executable agent plans. Prady Research applies classic Goal-Oriented Action Planning (GOAP) — A* search through a state space of actions with preconditions and effects — to autonomous AI research, then dispatches the work to live agents you can inspect in real time.

## Highlights

| | |
|---|---|
| 🎯 **Plain-English goals** | Describe an outcome — Prady extracts success criteria, constraints, and implicit preconditions |
...
```html
<div id="kryos-research-widget-container"></div>
<script>
  window.KryosResearchWidgetConfig = {
    primaryColor: "#8b5cf6",
    accentColor: "#10b981",
  };
</script>
...
The widget exposes a global `window.KryosResearchWidget` with `init(containerId)` and `version` for programmatic control. See [`docs/WIDGET-INTEGRATION.md`](docs/WIDGET-INTEGRATION.md) for the full integration guide.
