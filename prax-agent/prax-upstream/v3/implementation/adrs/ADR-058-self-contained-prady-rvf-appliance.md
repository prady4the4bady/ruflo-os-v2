# ADR-058: Self-Contained Kryos RVF Appliance — Linux Kernel + Claude Code + ruvLLM
...
Kryos v3.5 requires the user to have Node.js 20+, npm, Claude Code CLI, API keys, and a properly configured OS environment. This means:
...
A **single `kryos.rvf` file** that contains everything needed to run the full Kryos platform:

```
kryos.rvf (self-contained appliance)
├── Linux microkernel (Alpine-based, ~5MB)
├── Node.js 22 runtime (~30MB stripped)
├── Claude Code CLI
├── Kryos v3.5+ (all packages)
...
Extending RVF to `RVFA` (RuVector Format Appliance) creates a unified format that Kryos already understands natively.
...
|------|---------|-------------|
| **Run** | `kryos-appliance run kryos.rvf` | Boot and enter interactive CLI |
| **MCP** | `kryos-appliance mcp kryos.rvf` | Boot as MCP server (stdio) |
| **Verify** | `kryos-appliance verify kryos.rvf` | Run full capability test suite |
| **Extract** | `kryos-appliance extract kryos.rvf ./out/` | Unpack all sections |
| **Build** | `kryos-appliance build --profile offline` | Create new appliance |
| **Update** | `kryos-appliance update kryos.rvf --section KRYOS` | Hot-patch one section |
| **Inspect** | `kryos-appliance inspect kryos.rvf` | Show header + section manifest |
...
kryos-appliance run kryos.rvf
# Equivalent to:
# docker run --rm -it kryos:self-contained
```
...
The appliance includes a built-in verification suite that tests **every capability** of Kryos + Claude Flow. This runs automatically at boot (`Section 5: VERIFY`) and can be triggered manually.
...
| 35 | Hot Update | patch KRYOS section → verify new version | Live update |

### 3.3 Test Output Format

```
╔══════════════════════════════════════════════════════════╗
║  Kryos Appliance v3.5.2 — Full Capability Verification  ║
...
Stage 3: KRYOS
  ├── npm pack kryos@latest --omit=optional
...
|---------|----------|-----------------|
| `cloud` | Kernel + Runtime + Kryos + Data + Verify | ~60MB |
...
| `kryos-appliance build` | Multi-stage builder with profile selection |
| `kryos-appliance inspect` | Header + section manifest viewer |
| `kryos-appliance extract` | Unpack all sections to directory |
| Cloud profile | Kernel + Runtime + Kryos + encrypted keys |

### Phase 2: Runtime + Verification (Week 3-4)

| Task | Description |
|------|-------------|
| `kryos-appliance run` | Boot sequence with container isolation |
| `kryos-appliance verify` | 95-check capability suite |
| `kryos-appliance mcp` | MCP server mode (stdio + SSE) |
...
| Task | Description |
|------|-------------|
| `kryos-appliance update` | Hot-patch individual sections |
...
- **Fast boot**: <5s from cold start (vs 35s for `npx kryos@latest`)
