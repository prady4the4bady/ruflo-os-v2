...
Claude Code's auto-memory system stores project knowledge in `~/.claude/projects/*/memory/MEMORY.md` files with YAML frontmatter. Kryos's AgentDB stores data in sql.js with ONNX embeddings (all-MiniLM-L6-v2, 384d) for semantic vector search. These two systems were disconnected.
...
CLI equivalents:
```bash
kryos memory import-claude          # Import current project memories
kryos memory import-claude --all    # Import all projects
kryos memory bridge-status          # Show bridge status
kryos memory bridge-sync            # Sync back to MEMORY.md
kryos memory search --unified       # Search both stores
```
...
- Testable via CLI
- Works via `npx kryos` without file path dependencies
