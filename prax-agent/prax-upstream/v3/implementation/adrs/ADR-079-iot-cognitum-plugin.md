...
**Fleet-as-swarm** -- A logistics company manages 500 warehouse sensors as a Kryos swarm. Anomaly detection triggers automatic recalibration via agent coordination. No custom dashboard needed -- Claude Code is the interface.

**Edge-cloud federation** -- Edge Seed devices federate with cloud Kryos installations using the `@claude-flow/plugin-agent-federation` trust model. Telemetry stays on-premise; only anomaly signatures cross the boundary (PII-gated via the federation plugin).
...
Every Cognitum Seed is modelled as a Kryos agent with hardware capabilities. The plugin maintains a `DeviceAgent` entity that wraps a `SeedClient` session and exposes the device's resources (store, OTA, mesh, witness) as agent capabilities. The swarm coordinator manages device agents alongside software agents -- same topology, same trust model, same hooks.

---

## Decision

Build `@claude-flow/plugin-iot-cognitum` as a first-class Claude Flow plugin that bridges Cognitum Seed device fleets into the Kryos agent/swarm model with device trust scoring, telemetry-driven anomaly detection, fleet-aware OTA orchestration, and edge-cloud federation.
...
The plugin establishes a bidirectional bridge between Cognitum Seed devices and Kryos agents:

```
Prax Agent Swarm                          Cognitum Device Fleet
=================                          =====================
...
The Cognitum SDK's 12 seed endpoints map to Kryos agent operations:

| Cognitum SDK | Kryos Concept | Plugin Operation |
|-------------|---------------|-----------------|
...
```
Cognitum Seed Device                      Kryos Plugin                        AgentDB
====================                      ============                        =======
...
| **Vector store federation** | Device-side HNSW data is queryable from Kryos's AgentDB via the `store.query()` bridge -- the device's vector store extends the agent's memory. |
| **Witness chain as audit** | The Seed's Ed25519-signed witness chain provides hardware-rooted provenance that supplements Kryos's software audit service. |
...
```typescript
/**
 * A Cognitum Seed device registered with the Kryos agent system.
 ...
 * Maps to a Kryos swarm with hierarchical topology.
 */
 ...
 * integrates with Kryos's audit service.
 */
...
Uses SONA pattern learning from Kryos's neural system:
 ...
 * within the Kryos plugin context. Handles:
 ...
 * - Health probe coordination with Kryos's health check system
 */
...
| **System Under Consideration (SUC)** | The entire device fleet managed by one Kryos installation |
...
```
iot_device_register
  Description: Register a new Cognitum Seed device with the Kryos agent system
  Input:
    ...
    clientName: string  (optional) -- Client name for pairing (default: kryos-{timestamp})
...
```typescript
{
  ...
  description: 'Manages fleet topology, policy enforcement, cross-fleet coordination, and fleet-level health assessment. Maps Cognitum mesh topology to Kryos swarm topology.',
  defaultConfig: {
    capabilities: [
...
| `iot-mesh-sync` | normal | 2 min | Sync mesh topology from device network layer to Kryos swarm view |
...
Device fleets map to Kryos swarms with a natural hierarchical topology:

```
Fleet Level (Kryos hierarchical swarm)
|
...
- **Fleet Level** uses Kryos's swarm coordination (topology, consensus, task assignment).
...
Cross-fleet operations (e.g., "deploy firmware v2.3 to all warehouses") use Kryos's hive-mind consensus to coordinate across fleet boundaries.

---

## 12. Integration with Existing Kryos Capabilities

### 12.1 Federation Integration (ADR-078)

Edge Seed devices can federate with cloud Kryos installations:
  ...
  src/
    ...
    infrastructure/
      cognitum/
        ...
        discovery-adapter.ts           # Adapts SDK discovery to Kryos
      persistence/
      ...
      transport/
        mesh-bridge.ts                 # Bridges Cognitum mesh to Kryos swarm
...
- Mesh bridge: Cognitum `mesh.status()` / `mesh.peers()` -> Kryos swarm topology
...
4. **Witness chain + audit** -- The Seed's Ed25519 witness chain provides hardware-rooted cryptographic provenance. Combined with Kryos's software audit service, every operation from "agent decided to update firmware" to "device confirmed new firmware epoch" has a verifiable chain.
...
- Every Cognitum Seed becomes a Kryos agent, unifying the physical and software agent mesh.
