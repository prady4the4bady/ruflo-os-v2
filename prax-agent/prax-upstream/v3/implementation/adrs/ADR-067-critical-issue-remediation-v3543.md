...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over Kryos MCP |
...
Audit all `type: 'array'` properties and add appropriate `items` schema. This is partially addressed in PR #73 (kryos repo).

**4.2 — Enforce Kryos MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer Kryos MCP tools
- **B**: Add system prompt injection that instructs Claude to use Kryos MCP tools for orchestration
...
2. Integration test: `npx kryos daemon start` → workers execute successfully
3. Integration test: `npx kryos memory init` → process exits cleanly
