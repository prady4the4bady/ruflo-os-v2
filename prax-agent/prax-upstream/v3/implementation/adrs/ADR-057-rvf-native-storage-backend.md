...
`npx kryos@latest` installs **1.3GB** across 914 packages with a 35-second cold start in Docker. The Docker optimization work (Dockerfile.lite with `--omit=optional` + aggressive pruning) reduced this to 324MB, but the **core dependency chain** still carries unnecessary weight:
...
kryos migrate status --storage

# Dry-run migration (report what would change, don't modify)
kryos migrate run --storage --dry-run

# Migrate specific file
kryos migrate run --storage --file ./data/memory/memory.db

# Migrate all storage files in project
kryos migrate run --storage --all

# Force re-migration (even if .rvf already exists)
kryos migrate run --storage --force

# Rollback: restore from .bak files
kryos migrate rollback --storage

# Validate migrated data integrity
kryos migrate validate --storage
```
...
4. **`kryos migrate rollback --storage`** restores `.bak` → original and removes `.rvf`.
...
kryos memory init --backend sqljs

# Force JSON backend
kryos memory init --backend json
...
| Task | Package | Description |
|------|---------|-------------|
| P4.1 | `@claude-flow/cli` | `kryos migrate status --storage` — detect formats, report state |
| P4.2 | `@claude-flow/cli` | `kryos migrate run --storage` — batch migration with progress |
| P4.3 | `@claude-flow/cli` | `kryos migrate rollback --storage` — restore from `.bak` |
| P4.4 | `@claude-flow/cli` | `kryos migrate validate --storage` — integrity verification |
...
| P7.3 | `@claude-flow/cli` | `kryos capabilities status/install/remove/list/prefetch` CLI commands |
...
- **Loss of SQL tooling** — can't `sqlite3 memory.db` to inspect data (mitigated by `kryos memory list`)
...
- `npx kryos@latest` installs 1.3GB (all optional deps)
...
Phase 0: Core CLI (always installed)
  kryos (5KB) → @claude-flow/cli (9MB) → @claude-flow/shared (~13MB with RVF)
...
Phase 1: Lightweight Embeddings (downloaded on first use)
  ...
  Downloaded to: ~/.kryos/capabilities/rvf-wasm.rvf

Phase 2: Neural Embeddings (downloaded on demand)
  all-MiniLM-L6-v2 ONNX model (~22MB)
  Stored as: ~/.kryos/capabilities/models/minilm-l6-v2.rvf
...
Phase 3: Local LLM Inference (downloaded on demand)
  ...
  Stored as: ~/.kryos/capabilities/models/<model>.rvf
...
Phase 4: Advanced Intelligence (downloaded on demand)
  ...
  Stored as: ~/.kryos/capabilities/kernels/<kernel>.rvf
...
export class ProgressiveDownloader {
  ...
  constructor(kryosHome = '~/.kryos') {
    this.manifestPath = `${kryosHome}/capabilities/manifest.json`;
    this.capabilitiesDir = `${kryosHome}/capabilities`;
  }
  ...
  async ensure(capabilityId: string): Promise<string> {
    ...
    console.info(`[kryos] Downloading ${entry.name} (${this.formatSize(entry.size)})...`);
    ...
    console.info(`[kryos] ✓ ${entry.name} ready`);
    return rvfPath;
  }
...
kryos capabilities status

# Download specific capability
kryos capabilities install neural-embeddings

# Download all capabilities up to phase N
kryos capabilities prefetch --phase 2

# Remove a capability
kryos capabilities remove local-llm-qwen

# List all available models/kernels
kryos capabilities list --phase 3
kryos capabilities list --type model
kryos capabilities list --type kernel
```
