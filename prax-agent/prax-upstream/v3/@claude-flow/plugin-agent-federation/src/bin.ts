 ...
 * Maps `kryos-federation <subcommand> [args]` -> CLICommandDefinition handlers.
 *
...
function printHelp(commandNames: string[]): void {
  console.log('kryos-federation — Cross-installation agent federation CLI');
  console.log('');
  console.log('Usage: kryos-federation <subcommand> [options]');
  console.log('');
  ...
  console.log('');
  console.log('Run "kryos-federation <subcommand> --help" for command-specific options.');
}
...
async function main(): Promise<void> {
  ...
  if (!resolved) {
    ...
    console.error('Run "kryos-federation --help" to list available subcommands.');
    process.exitCode = 1;
    return;
  }
