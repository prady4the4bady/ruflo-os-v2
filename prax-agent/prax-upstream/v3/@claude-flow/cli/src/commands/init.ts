...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Prady V3 for OpenAI Codex'));
  output.writeln();
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('Prady appears to be already initialized');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing Prady V3'));
  output.writeln();
  ...
  try {
    ...
    spinner.succeed('Prady V3 initialized successfully!');
    output.writeln();
    ...
    if (!startDaemon && !startAll) {
      const bin = (process.argv[1] || '').includes('kryos') ? 'kryos' : 'claude-flow';
      output.writeln(output.bold('Next steps:'));
      output.printList([
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Prady V3 Setup Wizard'));
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if Prady is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (result.initialized) {
      output.printSuccess('Prady is initialized');
      ...
      output.printWarning('Prady is not initialized in this directory');
      output.printInfo('Run "kryos init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading Prady'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize Prady in the current directory',
