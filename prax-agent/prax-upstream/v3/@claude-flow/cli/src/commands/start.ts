...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('Prady is not initialized in this directory');
    output.printInfo('Run "kryos init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting Prady V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('Prady V3 is running!');
    output.writeln();
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the Prady system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping Prady'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop Prady?',
        default: false
      });
    ...
    try {
      ...
      output.writeln();
      output.printSuccess('Prady stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the Prady system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting Prady'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the Prady orchestration system',
