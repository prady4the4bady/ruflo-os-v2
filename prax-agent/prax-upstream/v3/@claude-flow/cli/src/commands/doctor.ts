  ...
  for (const configPath of mcpConfigPaths) {
    if (existsSync(configPath)) {
      try {
        ...
        const hasClaudeFlow = 'claude-flow' in servers || 'claude-flow_alpha' in servers || 'kryos' in servers || 'kryos_alpha' in servers;
        if (hasClaudeFlow) {
          return { name: 'MCP Servers', status: 'pass', message: `${count} servers (kryos configured)` };
        } else {
          return { name: 'MCP Servers', status: 'warn', message: `${count} servers (kryos not found)`, fix: 'claude mcp add kryos -- npx -y kryos@latest mcp start' };
        }
      } catch {
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@claude-flow/cli' || pkg.name === 'claude-flow' || pkg.name === 'kryos')
            ) {
...
export const doctorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Prady Doctor'));
