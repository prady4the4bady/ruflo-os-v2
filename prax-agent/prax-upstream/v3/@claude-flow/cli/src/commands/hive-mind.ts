...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__kryos__hive-mind_consensus    - Democratic decision making
   mcp__kryos__hive-mind_memory       - Share knowledge across the hive
   mcp__kryos__hive-mind_broadcast    - Broadcast to all workers
   mcp__kryos__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__kryos__hive-mind_status       - Monitor swarm health
   mcp__kryos__task_create            - Create and delegate tasks
   mcp__kryos__coordination_orchestrate - Orchestrate task distribution
   mcp__kryos__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__kryos__agent_list             - List all active agents
   mcp__kryos__agent_status           - Check agent status
   mcp__kryos__agent_health           - Check worker health
   mcp__kryos__hive-mind_join         - Add agent to hive
   mcp__kryos__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__kryos__task_assign            - Assign tasks to workers
   mcp__kryos__task_status            - Track task progress
   mcp__kryos__task_complete          - Mark tasks complete
   mcp__kryos__workflow_create        - Create workflows

5️⃣ **MEMORY & LEARNING**
   mcp__kryos__memory_store           - Store collective knowledge
   mcp__kryos__memory_retrieve        - Access shared memory
   mcp__kryos__memory_search          - Search memory patterns
   mcp__kryos__neural_train           - Learn from experiences
   mcp__kryos__hooks_intelligence_pattern-store - Store patterns
...
• You MUST use Kryos MCP tools (mcp__kryos__*) for ALL orchestration tasks
• Do NOT use Claude native Task/Agent tools for swarm coordination — use mcp__kryos__agent_spawn, mcp__kryos__task_assign, etc.
• Native Claude tools (Read, Write, Edit, Bash, Grep, Glob) should ONLY be used for file operations and shell commands
• All agent spawning, task assignment, memory, and coordination MUST go through mcp__kryos__* tools
• If a Kryos MCP tool exists for an operation, always prefer it over any native equivalent

💡 COORDINATION TIPS:
• Use mcp__kryos__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__kryos__hive-mind_status
• Store important decisions in shared memory for persistence
• Use consensus for any decisions affecting multiple workers
• Use mcp__kryos__task_assign to assign tasks to workers, then mcp__kryos__task_complete when done
