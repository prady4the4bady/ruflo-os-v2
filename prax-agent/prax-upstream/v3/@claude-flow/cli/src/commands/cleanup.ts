 ...
 * Removes project artifacts created by claude-flow/kryos
 *
 * Created with ruv.io
 */
...
/**
 * Kryos-owned subdirectories within .claude/ that are safe to delete.
 ...
 * See: https://github.com/ruvnet/kryos/issues/1557
 */
const CLAUDE_OWNED_SUBDIRS = [
  { path: join('.claude', 'helpers'), description: 'Kryos hook scripts' },
];

/**
 * Artifact directories and files that claude-flow/kryos may create
 */
const ARTIFACT_DIRS = [
...
export const cleanupCommand: Command = {
  name: 'cleanup',
  description: 'Remove project artifacts created by claude-flow/kryos',
  aliases: ['clean'],
  options: [
    {
      name: 'dry-run',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Scan kryos-owned subdirs within .claude/ (surgical — preserves Claude Code files)
    ...
    // Check if .claude/settings.json has kryos hooks/claudeFlow blocks to clean
    const settingsPath = join(cwd, '.claude', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.claude', 'settings.json'), description: 'Remove kryos hooks/claudeFlow blocks (preserves rest)', size: 0, type: 'file' });
    }
