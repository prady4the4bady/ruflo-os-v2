...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained kryos.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'kryos.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'kryos.rvf';
    ...
    const steps = [
      ...
      'Packaging kryos CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'kryos'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'kryos', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'kryos appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'kryos appliance inspect -f kryos.rvf', description: 'Inspect appliance contents' },
    { command: 'kryos appliance verify -f kryos.rvf', description: 'Verify integrity' },
    { command: 'kryos appliance extract -f kryos.rvf', description: 'Extract sections' },
    { command: 'kryos appliance run -f kryos.rvf', description: 'Boot and run appliance' },
    { command: 'kryos appliance sign -f kryos.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'kryos appliance publish -f kryos.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'kryos appliance update -f kryos.rvf -s kryos -d ./new-kryos.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Kryos Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full Kryos platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained kryos.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "kryos appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
