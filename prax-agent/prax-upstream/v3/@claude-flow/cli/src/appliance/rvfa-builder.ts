 ...
 *
 * Creates a single binary containing kernel, runtime, Kryos CLI, models/keys,
...
const KRYOS_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'kryos' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      kryosVersion: options.kryosVersion || detectKryosVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'kryos',   raw: this.buildKryosSection(),   label: 'Kryos CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> kryos-init (PID 1)',
  ...
  private buildKryosSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack kryos@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'kryos', version: this.opts.kryosVersion,
      package: packageMeta ?? { name: 'kryos', version: this.opts.kryosVersion },
      commands: KRYOS_COMMANDS, commandCount: KRYOS_COMMANDS.length,
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'KRYOS_CMD="${KRYOS_CMD:-kryos}"',
        'echo "Running basic verification..."',
        '$KRYOS_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$KRYOS_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      entrypoint: '/opt/kryos/bin/cli.js',
    ...
    return {
      magic: 'RVFA', version: 1, name: 'kryos-appliance', appVersion: this.opts.kryosVersion,
...
function detectKryosVersion(): string {
  try {
