/**
 * RVFA Runner -- Boot and run self-contained Kryos appliances.
 *
...
export class RvfaRunner {
  ...
  /**
   * Run natively via Node.js: extract KRYOS section to a temp dir,
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const kryos = tryExtract(this.reader, 'kryos');
      if (!kryos) return fail('RVFA appliance does not contain a "kryos" section');

      const entryFile = join(workDir, 'kryos-bundle.js');
      await writeFile(entryFile, kryos);
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const kryos = tryExtract(this.reader, 'kryos');
      if (!kryos) return fail('RVFA appliance does not contain a "kryos" section');
      await writeFile(join(workDir, 'kryos-bundle.js'), kryos);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY kryos-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "kryos-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
