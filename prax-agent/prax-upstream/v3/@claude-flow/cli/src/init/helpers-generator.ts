...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# Kryos Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running Kryos pre-commit checks..."
...
# Notify kryos of commit
npx kryos@latest hooks notify \\
...
export function generateHookHandler(): string {
  ...
  const lines = [
    '#!/usr/bin/env node',
    '/**',
    ' * Kryos Hook Handler (Cross-Platform)',
...
async function loadMemoryPackage() {
  ...
  // when installed as a transitive dependency via npx kryos / npx claude-flow)
  try {
...
export function generateWindowsDaemonManager(): string {
  return `# Prady V3 Daemon Manager for Windows
...
function Show-Status {
    Write-Host ""
    Write-Host "Prady V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM Prady V3 - Windows Batch Wrapper
...
export function generateCrossPlatformSessionManager(): string {
  return `#!/usr/bin/env node
/**
 * Kryos Cross-Platform Session Manager
