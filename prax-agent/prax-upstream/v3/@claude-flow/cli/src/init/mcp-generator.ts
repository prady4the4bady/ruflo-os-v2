...
export function generateMCPConfig(options: InitOptions): object {
  ...
  // Claude Flow MCP server (core) — uses kryos wrapper for portable npm-resolved invocation
  if (config.claudeFlow) {
    mcpServers['claude-flow'] = createMCPServerEntry(
      ['kryos@latest', 'mcp', 'start'],
      {
        ...npmEnv,
        CLAUDE_FLOW_MODE: 'v3',
...
export function generateMCPCommands(options: InitOptions): string[] {
  ...
  if (isWindows()) {
    if (config.claudeFlow) {
      commands.push('claude mcp add claude-flow -- cmd /c npx -y kryos@latest mcp start');
    }
    if (config.ruvSwarm) {
    ...
    if (config.claudeFlow) {
      commands.push("claude mcp add claude-flow -- npx -y kryos@latest mcp start");
    }
    if (config.ruvSwarm) {
