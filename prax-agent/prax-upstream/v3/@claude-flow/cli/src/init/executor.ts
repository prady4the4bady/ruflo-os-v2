  ...
  try {
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use Kryos'
      };
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Kryos'
      };
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add kryos allow rules)
...
async function writeRuntimeConfig(
  ...
  const config = `# Prady V3 Runtime Configuration
...
async function writeInitialMetrics(
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Kryos. Run: npx kryos@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
...
/**
 * Write CAPABILITIES.md - comprehensive overview of all Kryos features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilities = `# Prady V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
...
Prady V3 is a domain-driven design architecture for multi-agent AI coordination with:
...
\`\`\`bash
# Add Kryos MCP
claude mcp add kryos -- npx -y kryos@latest
...
# Swarm
npx kryos@latest swarm init --topology hierarchical --max-agents 8
npx kryos@latest swarm status
...
# Memory
npx kryos@latest memory search --query "patterns"

# Hooks
npx kryos@latest hooks pre-task --description "task"
npx kryos@latest hooks worker dispatch --trigger optimize
\`\`\`
...
async function writeClaudeMd(
  ...
  // Also write/append global ~/.claude/CLAUDE.md so kryos tools are used automatically (#1497)
  ...
  if (homeDir) {
    ...
    const kryosBlock = [
      '',
      '# Kryos Integration (auto-generated by kryos init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke kryos MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('Kryos Integration')) {
          fs.appendFileSync(globalClaudeMd, kryosBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended kryos block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, kryosBlock.trimStart(), 'utf-8');
