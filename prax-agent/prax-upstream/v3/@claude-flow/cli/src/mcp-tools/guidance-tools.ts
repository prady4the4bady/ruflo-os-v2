 ...
 * Guidance MCP Tools
 *
 * Helps the system navigate Kryos's capabilities by providing structured
...
const guidanceCapabilities: MCPTool = {
  ...
  description: 'List all capability areas with their tools, commands, agents, and skills. Use this to discover what Kryos can do.',
  inputSchema: {
    type: 'object',
    properties: {
      area: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const refs: Record<string, { title: string; commands: Array<{ cmd: string; desc: string }> }> = {
      'getting-started': {
        ...
        commands: [
          { cmd: 'npx kryos@latest init --wizard', desc: 'Initialize project with interactive setup' },
          { cmd: 'npx kryos@latest doctor --fix', desc: 'Run diagnostics and auto-fix issues' },
          { cmd: 'npx kryos@latest daemon start', desc: 'Start background workers' },
          { cmd: 'npx kryos@latest status', desc: 'Check system status' },
        ],
      },
      'daily-dev': {
        title: 'Daily Development',
        commands: [
          { cmd: 'npx kryos@latest hooks pre-task --description "..."', desc: 'Get routing recommendation before task' },
          { cmd: 'npx kryos@latest hooks post-task --task-id "..." --success true', desc: 'Record task outcome for learning' },
          { cmd: 'npx kryos@latest hooks post-edit --file "..." --train-neural true', desc: 'Train patterns from edits' },
          { cmd: 'npx kryos@latest memory search --query "..."', desc: 'Search memory for relevant patterns' },
          { cmd: 'npx kryos@latest hooks route --task "..."', desc: 'Route task to optimal agent' },
        ],
      },
      'swarm-ops': {
        title: 'Swarm Operations',
        commands: [
          { cmd: 'npx kryos@latest swarm init --topology hierarchical --max-agents 8', desc: 'Initialize anti-drift swarm' },
          { cmd: 'npx kryos@latest swarm status', desc: 'Check swarm status' },
          { cmd: 'npx kryos@latest agent spawn -t coder --name my-coder', desc: 'Spawn a specific agent' },
          { cmd: 'npx kryos@latest hive-mind init --strategy byzantine', desc: 'Start hive-mind consensus' },
        ],
      },
      'memory-ops': {
        title: 'Memory Operations',
        commands: [
          { cmd: 'npx kryos@latest memory init --force', desc: 'Initialize memory database' },
          { cmd: 'npx kryos@latest memory store --key "k" --value "v" --namespace patterns', desc: 'Store a value' },
          { cmd: 'npx kryos@latest memory search --query "auth patterns"', desc: 'Semantic vector search' },
          { cmd: 'npx kryos@latest memory list --namespace patterns', desc: 'List entries in namespace' },
          { cmd: 'npx kryos@latest memory retrieve --key "k" --namespace patterns', desc: 'Get a specific entry' },
        ],
      },
      'github-ops': {
      ...
      diagnostics: {
        ...
        commands: [
          { cmd: 'npx kryos@latest doctor --fix', desc: 'Full system diagnostics with auto-fix' },
          { cmd: 'npx kryos@latest status --watch', desc: 'Live system monitoring' },
          { cmd: 'npx kryos@latest hooks worker status', desc: 'Background worker health' },
          { cmd: 'npx kryos@latest performance benchmark --suite all', desc: 'Run all benchmarks' },
          { cmd: 'npx kryos@latest hooks progress --detailed', desc: 'V3 implementation progress' },
        ],
      },
    };
