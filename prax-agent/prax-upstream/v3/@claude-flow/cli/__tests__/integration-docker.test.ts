 ...
 *
 * Validates the Prady Docker-based deployment stack without running Docker.
...
const KRYOS_DIR = join(ROOT, 'kryos');
const COMPOSE_PATH = join(KRYOS_DIR, 'docker-compose.yml');
const NGINX_CONF_PATH = join(KRYOS_DIR, 'src', 'nginx', 'nginx.conf');
const NGINX_DOCKERFILE = join(KRYOS_DIR, 'src', 'nginx', 'Dockerfile');
const MCP_BRIDGE_INDEX = join(KRYOS_DIR, 'src', 'mcp-bridge', 'index.js');
const MCP_BRIDGE_DOCKERFILE = join(KRYOS_DIR, 'src', 'mcp-bridge', 'Dockerfile');
const CLI_DOCKERFILE = join(CLI_DIR, 'docker', 'Dockerfile');
const ENV_EXAMPLE = join(KRYOS_DIR, '.env.example');
...
describe('Nginx Configuration', () => {
  ...
  it('injects Prady welcome.js script via sub_filter', () => {
    expect(nginxContent).toMatch(/sub_filter\s+'<\/head>'\s+'<script src="\/kryos\/welcome\.js"/);
  });
  ...
  it('serves /kryos/ static assets from /etc/nginx/static/', () => {
    expect(nginxContent).toMatch(/location\s+\/kryos\//);
  });
...
describe('CLI Dockerfile (kryos:lite)', () => {
  let dockerContent: string;
  ...
  it('installs kryos globally in the build stage', () => {
    expect(dockerContent).toContain('npm install -g kryos@latest');
  });
  ...
  it('creates a non-root user (kryos)', () => {
    expect(dockerContent).toContain('adduser');
    expect(dockerContent).toContain('kryos');
    expect(dockerContent).toContain('USER kryos');
  });
  ...
  it('has a HEALTHCHECK using kryos doctor', () => {
    expect(dockerContent).toContain('HEALTHCHECK');
    expect(dockerContent).toContain('kryos doctor');
  });

  it('default CMD starts MCP server', () => {
    expect(dockerContent).toContain('CMD ["kryos", "mcp", "start"]');
  });
});
...
describe('Security Checks', () => {
  ...
  it('CLI Dockerfile runs as non-root', () => {
    ...
    expect(dockerfile).toContain('USER kryos');
  });
