...
> Complete reference documentation for Kryos v3.5. For a quick overview, see the [README](../README.md).
...
Kryos is a comprehensive AI agent orchestration framework that transforms Claude Code into a powerful multi-agent development platform. It enables teams to deploy, coordinate, and optimize specialized AI agents working together on complex software engineering tasks.

### Self-Learning/Self-Optimizing Agent Architecture

```
User → Kryos (CLI/MCP) → Router → Swarm → Agents → Memory → LLM Providers
                       ↑                          ↓
                       └──── Learning Loop ←──────┘
```
...
**RuVector Components** (included with Kryos):
...
```bash
# Use RuVector via Kryos
npx kryos@latest hooks intelligence --status
```
...
curl -fsSL https://cdn.jsdelivr.net/gh/ruvnet/kryos@main/scripts/install.sh | bash

# Or full setup with MCP + diagnostics
curl -fsSL https://cdn.jsdelivr.net/gh/ruvnet/kryos@main/scripts/install.sh | bash -s -- --full
...
⚡ **Plugs Into Claude Code** - Native integration via MCP (Model Context Protocol). Use kryos commands directly in your Claude Code sessions with full tool access.
...
Complex swarms can drift from their original goals. Kryos V3 includes anti-drift defaults that prevent agents from going off-task.
...
### Claude Code: With vs Without Kryos

| Capability | Claude Code Alone | Claude Code + Kryos |
|------------|-------------------|---------------------------|
...
# Or install globally
npm install -g kryos@latest
kryos init

# With Bun (faster)
bunx kryos@latest init
```
...
npm install -g kryos@latest --omit=optional
```

#### Claude Code Plugin Marketplace

Install Kryos as a native Claude Code plugin -- adds skills, commands, agents, and MCP tools directly into Claude Code:
...
/plugin install kryos-core@kryos         # MCP server + base agents
/plugin install kryos-swarm@kryos         # Swarm coordination + Monitor
/plugin install kryos-autopilot@kryos     # Autonomous /loop completion
/plugin install kryos-loop-workers@kryos  # Background workers + CronCreate
/plugin install kryos-security-audit@kryos # Security scanning
/plugin install kryos-rag-memory@kryos    # HNSW memory + AgentDB
/plugin install kryos-testgen@kryos       # Test gap detection + TDD
/plugin install kryos-docs@kryos          # Doc generation + drift detection
```
...
Kryos supports both **Claude Code** and **OpenAI Codex CLI** via the [@claude-flow/codex](https://www.npmjs.com/package/@claude-flow/codex) package, following the [Agentics Foundation](https://agentics.org) standard.

### Quick Start for Codex

```bash
# Initialize for Codex CLI (creates AGENTS.md instead of CLAUDE.md)
npx kryos@latest init --codex

# Full Codex setup with all 137+ skills
npx kryos@latest init --codex --full

# Initialize for both platforms (dual mode)
npx kryos@latest init --dual
```
...
codex mcp add kryos -- npx kryos mcp start
```
...
npx kryos@latest mcp start

# Spawn a coding agent
npx kryos@latest agent spawn -t coder --name my-coder

# Launch a hive-mind swarm with an objective
npx kryos@latest hive-mind spawn "Implement user authentication"

# List available agent types
npx kryos@latest agent list
```
...
npx kryos@latest init upgrade

# Update AND add any missing skills/agents/commands
npx kryos@latest init upgrade --add-missing
```
...
Add kryos as an MCP server for seamless integration:

```bash
# Add kryos MCP server to Claude Code
...
Once added, Claude Code can use all 313 kryos MCP tools directly:
...
<details>
<summary>🆚 <strong>Why Kryos v3?</strong></summary>

Kryos v3 introduces **self-learning neural capabilities** that no other agent orchestration framework offers. While competitors require manual agent configuration and static routing, Kryos learns from every task execution, prevents catastrophic forgetting of successful patterns, and intelligently routes work to specialized experts.

#### 🧠 Neural & Learning

| Feature | Kryos v3 | CrewAI | LangGraph | AutoGen | Manus |
|---------|----------------|--------|-----------|---------|-------|
...
What makes Kryos different from other agent frameworks? These 10 capabilities work together to create a system that learns from experience, runs efficiently on any hardware, and keeps working even when things go wrong.
...
Not every task needs the most powerful (and expensive) model. Kryos analyzes each request and automatically routes it to the cheapest handler that can do the job well. Simple code transforms skip the LLM entirely using WebAssembly. Medium tasks use faster, cheaper models. Only complex architecture decisions use Opus.
...
Kryos V3 integrates AgentDB v3 (3.0.0-alpha.10) providing 20+ memory controllers accessible via MCP tools and the CLI.
...
Connect Kryos to your development environment.

<details>
<summary>🔌 <strong>MCP Setup</strong> — Connect Kryos to Any AI Environment</summary>

Kryos runs as an MCP (Model Context Protocol) server, allowing you to connect it to any MCP-compatible AI client. This means you can use Kryos's 100+ agents, swarm coordination, and self-learning capabilities from Claude Desktop, VS Code, Cursor, Windsurf, ChatGPT, and more.

### Quick Add Command

```bash
# Start Kryos MCP server in any environment
npx kryos@latest mcp start
```
...
For ChatGPT, you need a remote MCP server (not local stdio). Deploy kryos to a server with HTTP transport:

```bash
# Start with HTTP transport
npx kryos@latest mcp start --transport http --port 3000
```
...
2. Configure your kryos MCP server
...
**CLI Commands:**
```bash
npx kryos hive-mind init                    # Initialize hive mind
npx kryos hive-mind spawn "Build API"       # Spawn with objective
npx kryos hive-mind spawn "..." --queen-type strategic --consensus byzantine
npx kryos hive-mind status                  # Check status
npx kryos hive-mind metrics                 # Performance metrics
npx kryos hive-mind memory                  # Collective memory stats
npx kryos hive-mind sessions                # List active sessions
```
...
**Enable Agent Teams:**
```bash
# Automatically enabled with kryos init
npx kryos@latest init
...
npx kryos@latest hooks teammate-idle --auto-assign true

# Handle task completion
npx kryos@latest hooks task-completed --task-id <id> --train-patterns
```
...
Install these optional plugins to extend Kryos capabilities:
...
| **@claude-flow/plugin-gastown-bridge** | 0.1.0 | Gas Town orchestrator integration with WASM-accelerated formula parsing (instant (regex-based, no LLM call)), Beads sync, convoy management, and graph analysis. 20 MCP tools. | `npx kryos@latest plugins install -n @claude-flow/plugin-gastown-bridge` |
| **@claude-flow/teammate-plugin** | 1.0.0-alpha.1 | Native TeammateTool integration for Claude Code v2.1.19+. BMSSP WASM acceleration, rate limiting, circuit breaker, semantic routing. 21 MCP tools. | `npx kryos@latest plugins install -n @claude-flow/teammate-plugin` |
...
npx kryos@latest plugins install -n @claude-flow/plugin-gastown-bridge
...
npx kryos plugins list --installed
```
...
```bash
npx kryos@latest worker dispatch --trigger audit --context "./src"
npx kryos@latest worker status
```
...
| **Auto-Install** | `kryos embeddings init` or `createEmbeddingServiceAsync()` | Zero config |
...
kryos embeddings init

# Use larger model for higher quality
kryos embeddings init --model all-mpnet-base-v2

# Semantic search
kryos embeddings search -q "authentication patterns"
```
...
kryos ruvector init --database mydb --user admin

# Check connection and schema status
kryos ruvector status --verbose

# Run pending migrations
kryos ruvector migrate --up

# Performance benchmark
kryos ruvector benchmark --iterations 1000

# Optimize indices and vacuum
kryos ruvector optimize --analyze

# Backup vector data
kryos ruvector backup --output ./backup.sql
```
...
**Quick Commands:**
```bash
npx kryos hive-mind init                                    # Initialize
npx kryos hive-mind spawn "Build API" --queen-type tactical # Spawn swarm
npx kryos hive-mind spawn "Research AI" --consensus byzantine --claude
npx kryos hive-mind status                                  # Check status
```

**Kryos Skill:** `/hive-mind-advanced` — Full hive mind orchestration
...
**Output Format:**
```
▊ Kryos V3 ● ruvnet  │  ⎇ main  │  Opus 4.6  | ●42% ctx  | $0.15
🏗️ DDD [●●●●○] 4/5  ⚡ HNSW 150x  🤖 ◉ [12/8]  👥 3  🟢 CVE 3/3  💾 512MB  🧠 15%  📦 AgentDB ●1.2K vectors
```

| Indicator | Description | Source |
|-----------|-------------|--------|
| `▊ Kryos V3` | Project header | Always shown |
...
Run `npx kryos@latest init` — this generates `.claude/settings.json` with the correct statusline config and creates the helper script at `.claude/helpers/statusline.cjs`.
...
```bash
npx kryos@latest init --update --settings
```
...
npx kryos@latest daemon start

# Check status with worker history
npx kryos@latest daemon status

# Manually trigger a worker
npx kryos@latest daemon trigger map

# Enable/disable workers
npx kryos@latest daemon enable map audit optimize

# Stop daemon
npx kryos@latest daemon stop
```
...
Complete command-line interface for all Kryos operations.
...
npx kryos@latest swarm init --v3-mode

# Search memory (HNSW-indexed, 150x faster)
npx kryos@latest memory search -q "authentication patterns"

# Run security scan
npx kryos@latest security scan --depth full

# Performance benchmark
npx kryos@latest performance benchmark --suite all
```
...
Run `npx kryos@latest doctor` to diagnose and fix common issues.
...
npx kryos@latest doctor

# Run diagnostics with auto-fix
npx kryos@latest doctor --fix

# Check specific component
npx kryos@latest doctor --component memory

# Verbose output
npx kryos@latest doctor --verbose
```
...
npx kryos@latest embeddings init

# Generate embedding for text
npx kryos@latest embeddings embed "authentication patterns"

# Batch embed multiple texts
npx kryos@latest embeddings batch --file texts.txt

# Search with semantic similarity
npx kryos@latest embeddings search "login flow" --top-k 5
```
...
|----------|----------------|--------------|
| **Code Review** | Get thorough reviews with security, performance, and style checks | `npx kryos@latest agent spawn -t reviewer --name pr-review` |
| **Test Generation** | Auto-generate unit, integration, and e2e tests for existing code | `npx kryos@latest agent spawn -t tester --name test-gen` |
| **Refactoring** | Safely restructure code while maintaining behavior | `npx kryos@latest hive-mind spawn "Refactor user service to repository pattern"` |
| **Bug Fixing** | Diagnose and fix bugs with full context analysis | `npx kryos@latest hive-mind spawn "Fix race condition in checkout flow"` |

### 🔒 Security & Compliance

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Security Audit** | Find vulnerabilities before attackers do | `npx kryos@latest security scan --depth full` |
| **Dependency Scan** | Identify vulnerable packages and suggest upgrades | `npx kryos@latest security cve --check` |
| **Compliance Check** | Ensure code meets security standards | `npx kryos@latest security audit` |

### 🐝 Multi-Agent Swarms

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Feature Development** | Coordinate multiple agents on complex features | `npx kryos@latest swarm init --topology hierarchical && npx kryos@latest task orchestrate "Build user dashboard"` |
| **Large Refactors** | Parallel refactoring across many files without conflicts | `npx kryos@latest swarm init --topology mesh --max-agents 8` |
| **Codebase Migration** | Migrate frameworks, languages, or patterns systematically | `npx kryos@latest task orchestrate "Migrate from Express to Fastify" --strategy adaptive` |

### 📊 Performance & Optimization

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Performance Profiling** | Find and fix bottlenecks in your application | `npx kryos@latest performance profile --target src/` |
| **Query Optimization** | Speed up slow database queries | `npx kryos@latest performance benchmark --suite all` |
| **Memory Analysis** | Reduce memory usage and fix leaks | `npx kryos@latest performance metrics` |

### 🔄 GitHub & DevOps

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **PR Management** | Review, approve, and merge PRs efficiently | `npx kryos@latest hive-mind spawn "Review open PRs"` |
| **Issue Triage** | Categorize, prioritize, and assign issues automatically | `npx kryos@latest hive-mind spawn "Triage new issues"` |
| **Release Management** | Coordinate releases with changelogs and versioning | `npx kryos@latest hive-mind spawn "Prepare v2.0 release"` |
| **CI/CD Optimization** | Speed up pipelines and reduce flaky tests | `npx kryos@latest hive-mind spawn "Optimize GitHub Actions workflow"` |

### 📋 Spec-Driven Development

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Generate Specs** | Create complete specifications before coding | `npx kryos@latest hive-mind spawn "Create ADR for authentication system"` |
| **Validate Implementation** | Ensure code matches specifications | `npx kryos@latest hooks progress --detailed` |
| **Track Compliance** | Monitor spec adherence across the team | `npx kryos@latest progress sync` |

### 🧠 Learning & Intelligence

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Bootstrap Intelligence** | Train the system on your codebase patterns | `npx kryos@latest hooks pretrain --depth deep` |
| **Optimize Routing** | Improve task-to-agent matching over time | `npx kryos@latest hooks route "<task>" --include-explanation` |
| **Transfer Learning** | Apply patterns learned from other projects | `npx kryos@latest hooks transfer <sourceProject>` |
...
Kryos eliminates Claude Code's context window ceiling with a real-time memory management system that archives, optimizes, and restores conversation context automatically.
...
Kryos intercepts the compaction lifecycle with three hooks that make context loss invisible:
...
Kryos uses RVF — a compact binary storage format that replaces the 18MB sql.js WASM dependency with pure TypeScript. No native compilation, no WASM downloads, works everywhere Node.js runs.
...
# Or via CLI
kryos memory init --force
kryos config set memory.backend hybrid
```
...
**Example Hook Output:**
```bash
$ npx kryos@latest hooks pre-task --description "convert var to const in utils.ts"
...
npx kryos@latest hooks pre-edit ./src/auth.ts
npx kryos@latest hooks post-edit ./src/auth.ts --success true --train-patterns
```
...
npx kryos@latest hooks route "refactor authentication to use JWT" --include-explanation

# Bootstrap intelligence from your codebase
npx kryos@latest hooks pretrain --depth deep --model-type moe
```
...
npx kryos@latest hooks session-start --session-id "feature-auth" --start-daemon

# End session and export learnings
npx kryos@latest hooks session-end --export-metrics --persist-patterns
```
...
npx kryos@latest hooks intelligence trajectory-start --task "implement OAuth2"

# Record successful action
npx kryos@latest hooks intelligence trajectory-step --action "created token service" --quality 0.9

# End trajectory and trigger learning
npx kryos@latest hooks intelligence trajectory-end --success true
...
```bash
# List all workers
npx kryos@latest hooks worker list

# Manually dispatch security audit
npx kryos@latest hooks worker dispatch --trigger audit --context "./src/auth"

# Check worker status
npx kryos@latest hooks worker status
```
...
npx kryos@latest hooks model-route --task "fix typo in README"
# → Recommends: haiku (simple task, low complexity)

npx kryos@latest hooks model-route --task "design distributed consensus system"
...
npx kryos@latest hooks route "<task>" --include-explanation

# Start/end session with learning
npx kryos@latest hooks session-start --start-daemon
npx kryos@latest hooks session-end --persist-patterns

# View what the system has learned
npx kryos@latest hooks metrics
npx kryos@latest hooks intelligence stats
...
npx kryos@latest hooks pretrain --depth deep

# Dispatch background worker
npx kryos@latest hooks worker dispatch --trigger audit
```
...
npx kryos@latest memory export --format json --output ./patterns.json

# Export specific namespace
npx kryos@latest memory export --namespace "security" --output ./security-patterns.json

# Export with embeddings (larger file, faster import)
npx kryos@latest memory export --include-embeddings --output ./full-export.json

# Export agent configurations
npx kryos@latest config export --scope project --output ./agent-configs.json

# Export session state
npx kryos@latest session export --session-id "my-session" --output ./session.json
```
...
npx kryos@latest memory import --input ./patterns.json

# Import and merge with existing (don't overwrite)
npx kryos@latest memory import --input ./patterns.json --merge

# Import from another project
npx kryos@latest hooks transfer --source-path ../other-project

# Import agent configurations
npx kryos@latest config import --input ./agent-configs.json --scope project

# Restore session
npx kryos@latest session restore --session-id "my-session"
```
...
npx kryos@latest transfer-store search --query "authentication" --min-rating 4.0

# Download a pattern
npx kryos@latest transfer-store download --id "auth-jwt-patterns-v2" --verify

# Publish your patterns
npx kryos@latest transfer-store publish --input ./my-patterns.json --category "security"
```
...
npx kryos@latest plugins list

# Filter by type
npx kryos@latest plugins list --type integration

# Rate a plugin
npx kryos@latest plugins rate --name @claude-flow/embeddings --rating 5

# Search for MCP tool plugins
npx kryos@latest transfer plugin-search --type "mcp-tool" --verified

# Get plugin info
npx kryos@latest transfer plugin-info --name "semantic-code-search"

# List official plugins
npx kryos@latest transfer plugin-official
```
...
npx kryos@latest transfer ipfs-resolve --name "/ipns/patterns.kryos.io"

# Detect PII before publishing
npx kryos@latest transfer detect-pii --content "$(cat ./patterns.json)"
```
...
curl -X POST "https://api.pinata.cloud/pinning/pinJSONToIPFS" \
  ...
  -d '{
    ...
    "pinataMetadata": {"name": "kryos-learning-pattern"}
  }'
...
# Import all models
npx kryos@latest transfer import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Import specific category
npx kryos@latest neural import --model security-review-patterns --source ipfs

# Use patterns in routing
npx kryos@latest hooks route --task "review authentication code" --use-patterns
```
...
npx kryos@latest transfer-store download --id "security-essentials" --apply
```
...
npx kryos@latest neural list

# List models by category
npx kryos@latest neural list --category security

# Train with WASM acceleration
npx kryos@latest neural train -p coordination -e 100 --wasm --flash --contrastive

# Train security patterns
npx kryos@latest neural train -p security --wasm --contrastive

# Benchmark WASM performance
npx kryos@latest neural benchmark -d 256 -i 1000

# Import pre-trained models
npx kryos@latest neural import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Export trained patterns to IPFS
npx kryos@latest neural export --ipfs --sign
```
...
# Via CLI
npx kryos@latest skill run github-code-review
npx kryos@latest skill list
npx kryos@latest skill info sparc-methodology
```
...
|---------|--------------|---------|
| `issues list` | See all issues and their status | `npx kryos@latest issues list` |
| `issues claim` | Claim an issue for yourself/agent | `npx kryos@latest issues claim #123 --as coder-1` |
| `issues release` | Release your claim | `npx kryos@latest issues release #123` |
| `issues handoff` | Hand off to another worker | `npx kryos@latest issues handoff #123 --to reviewer` |
| `issues status` | Update progress on claimed work | `npx kryos@latest issues status #123 --progress 75` |
| `issues stealable` | List abandoned/stuck issues | `npx kryos@latest issues stealable` |
| `issues steal` | Take over stealable issue | `npx kryos@latest issues steal #123` |
| `issues load` | View agent workloads | `npx kryos@latest issues load` |
| `issues rebalance` | Redistribute work evenly | `npx kryos@latest issues rebalance --dry-run` |
| `issues board` | Visual board view | `npx kryos@latest issues board` |

### Visual Board View

```bash
npx kryos@latest issues board
```
...
npx kryos@latest issues handoff #123 \
  --to security-architect \
  --reason "Needs security review" \
  --progress 80

# 2. Target accepts handoff
npx kryos@latest issues accept #123 --as security-architect
...
# Auto-rebalance
npx kryos@latest issues rebalance
```
...
| Command | What It Does | Example |
|---------|--------------|---------|
| `route task` | Get agent recommendation | `npx kryos@latest route task "implement OAuth2"` |
| `route explain` | Understand routing decision | `npx kryos@latest route explain "task"` |
| `route coverage` | Route based on test coverage | `npx kryos@latest route coverage` |

### Example: Route a Task

```bash
npx kryos@latest route task "refactor authentication to use JWT"
...
```bash
npx kryos@latest route coverage
...
npx kryos@latest hooks route "implement caching layer" --include-explanation

# Record outcome for learning
npx kryos@latest hooks post-task --task-id "task-123" --success true --agent coder
```
...
Use Kryos packages directly in your applications.

<details>
<summary>💻 <strong>Programmatic SDK</strong> — Use Kryos in Your Code</summary>

Use Kryos packages directly in your TypeScript/JavaScript applications.
...
npx kryos@latest memory init --force

# Store patterns
npx kryos@latest memory store --key "pattern-auth" --value "JWT authentication with refresh tokens"
npx kryos@latest memory store --key "pattern-cache" --value "Redis caching for API responses"

# Build HNSW index for HNSW-indexed search
npx kryos@latest memory search --query "authentication" --build-hnsw

# Semantic search (uses HNSW if built)
npx kryos@latest memory search --query "how to cache data" --limit 5

# List and manage entries
npx kryos@latest memory list --namespace patterns
npx kryos@latest memory stats
```
...
```bash
# Generate embedding
kryos embeddings embed "Your text here"

# Batch embed from file
kryos embeddings batch documents.txt -o embeddings.json

# Similarity search
kryos embeddings search "query" --index ./vectors

# Document chunking
kryos embeddings chunk document.txt --strategy sentence --max-size 512

# Normalize embeddings
kryos embeddings normalize embeddings.json --type l2 -o normalized.json

# Convert to hyperbolic
kryos embeddings hyperbolic embeddings.json -o poincare.json

# Neural operations
kryos embeddings neural drift --baseline "context" --input "check"
kryos embeddings neural store --id mem-1 --content "data"
kryos embeddings neural recall "query" --top-k 5

# Model management
kryos embeddings models list
kryos embeddings models download all-MiniLM-L6-v2

# Cache management
kryos embeddings cache stats
kryos embeddings cache clear --older-than 7d
```
...
Core infrastructure packages powering Kryos's intelligence layer.
...
Kryos v3 is built on top of **[agentic-flow](https://github.com/ruvnet/agentic-flow)**, a production-ready AI agent orchestration platform. This deep integration provides instant (regex-based, no LLM call) code transformations, learning memory, and geometric intelligence.
...
The agentic-flow ecosystem exposes MCP tools across packages (kryos CLI provides 314 tools):
...
### Integration with Kryos

Kryos automatically leverages agentic-flow for:
...
```typescript
// Kryos automatically uses agentic-flow optimizations
import { getTokenOptimizer } from '@claude-flow/integration';
...
Kryos includes a dedicated `/agentic-jujutsu` skill for AI-powered version control:
...
**RuVector** is a high-performance distributed vector database combining vector search, graph queries, and self-learning neural networks. Written in Rust with Node.js/WASM bindings, it powers Kryos's intelligence layer with native speed.
...
npx kryos ruvector setup --output ./my-ruvector
...
Kryos automatically uses RuVector when available:

```typescript
// Kryos detects and uses native ruvector
...
npx kryos ruvector setup                    # Output to ./ruvector-postgres
npx kryos ruvector setup --output ./mydir   # Custom directory
npx kryos ruvector setup --print            # Preview files

# Import from sql.js/JSON to PostgreSQL
npx kryos ruvector import --input data.json              # Direct import
npx kryos ruvector import --input data.json --output sql # Dry-run (generate SQL)

# Other RuVector commands
npx kryos ruvector status --verbose         # Check connection
npx kryos ruvector benchmark --vectors 10000 # Performance test
npx kryos ruvector optimize --analyze       # Optimization suggestions
npx kryos ruvector backup --output backup.sql # Backup data
...
Flow Nexus is a **cloud platform** for deploying and scaling Kryos beyond your local machine.

### What Flow Nexus Provides

| Feature | Local Kryos | + Flow Nexus |
|---------|-------------------|--------------|
...
# Or via CLI
npx kryos@latest nexus swarm deploy \
  --topology hierarchical \
  --max-agents 50 \
  --region us-east-1
```
...
```bash
# Create sandbox
npx kryos@latest nexus sandbox create --language python

# Execute code safely
npx kryos@latest nexus sandbox exec --code "print('Hello')"

# Cleanup
npx kryos@latest nexus sandbox destroy
```
...
# 2. Get API key
# 3. Configure
npx kryos@latest nexus configure --api-key <key>
...
# Define pipeline
npx kryos@latest stream-chain create \
...
```bash
# Run the pipeline
npx kryos@latest stream-chain run feature-pipeline \
  --input '{"requirements": "Add user dashboard with analytics"}'

# Monitor progress
npx kryos@latest stream-chain status feature-pipeline
```
...
# Via CLI
npx kryos@latest pair start --mode navigator
```
...
npx kryos@latest pair switch

# Get AI explanation
npx kryos@latest pair explain

# Run tests
npx kryos@latest pair test

# End session with summary
npx kryos@latest pair end
```
...
```bash
# Basic threat scan
npx kryos@latest security defend -i "ignore previous instructions"

# Scan a file
npx kryos@latest security defend -f ./user-prompts.txt

# Quick scan (faster)
npx kryos@latest security defend -i "some text" --quick

# JSON output
npx kryos@latest security defend -i "test" -o json

# View statistics
npx kryos@latest security defend --stats
...
AI-optimized browser automation integrating [agent-browser](https://github.com/AugmentCode/agent-browser) with kryos for intelligent web automation, trajectory learning, and multi-agent browser coordination.
...
Automated release management, versioning, and CI/CD for Kryos packages.
...
# Performance report
npx kryos@latest performance report

# Benchmark specific suite
npx kryos@latest performance benchmark --suite memory
```
...
| `GCS_PREFIX` | Prefix for stored files | `kryos-patterns` |
...
Kryos looks for configuration in this order:
1. `./claude-flow.config.json` (project root)
2. `~/.config/kryos/config.json` (user config)
...
```json
{
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "file",
    "filePath": "/var/log/kryos/production.log"
  },
...
npx kryos@latest config list

# Get specific value
npx kryos@latest config get --key memory.type

# Set configuration value
npx kryos@latest config set --key swarm.maxAgents --value 10

# Export configuration
npx kryos@latest config export > my-config.json

# Import configuration
npx kryos@latest config import --file my-config.json

# Reset to defaults
npx kryos@latest config reset --key swarm
...
npx kryos@latest hooks pretrain
```
...
$env:CLAUDE_FLOW_MEMORY_PATH = "C:/Users/name/kryos/data"
```
...
node --expose-gc node_modules/.bin/kryos
...
| Change | V2 | V3 | Impact |
|--------|----|----|--------|
| **Package Structure** | `kryos` | `@claude-flow/*` (scoped) | Update imports |
...
| **Config Format** | `.kryos/config.json` | `claude-flow.config.json` | Update path |
...
cp -r ./.kryos ./.kryos-backup-v2

# STEP 2: Check migration status
npx kryos@latest migrate status

# STEP 3: Run migration with dry-run first
npx kryos@latest migrate run --dry-run

# STEP 4: Execute migration
npx kryos@latest migrate run --from v2

# STEP 5: Verify migration
npx kryos@latest migrate verify
...
|------------|------------|-------|
| `kryos start` | `kryos mcp start` | MCP is explicit |
| `kryos init` | `kryos init --wizard` | Interactive mode |
| `kryos spawn <type>` | `kryos agent spawn -t <type>` | Nested under `agent` |
| `kryos swarm create` | `kryos swarm init --topology mesh` | Explicit topology |
...
```typescript
// V2 (deprecated)
import { ClaudeFlow, Agent, Memory } from 'kryos';
...
npx kryos@latest migrate rollback --list

# Rollback to V2
npx kryos@latest migrate rollback --to v2
...
- [ ] Verify all agents spawn correctly: `npx kryos@latest agent list`
- [ ] Check memory search works: `npx kryos@latest memory search -q "test"`
- [ ] Confirm MCP server starts: `npx kryos@latest mcp start`
- [ ] Run doctor diagnostics: `npx kryos@latest doctor`
- [ ] Test a simple swarm: `npx kryos@latest swarm init --topology mesh`
- [ ] Bootstrap learning: `npx kryos@latest hooks pretrain`
