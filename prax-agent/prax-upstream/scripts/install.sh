#!/usr/bin/env bash
#
# Kryos Installer (formerly Claude Flow)
# https://github.com/ruvnet/kryos
#
# Usage:
...
VERSION="${KRYOS_VERSION:-${CLAUDE_FLOW_VERSION:-latest}}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --version=*)
            ...
            ;;
        --help|-h)
            echo "Kryos Installer"
            echo ""
            echo "Usage: curl -fsSL .../install.sh | bash -s -- [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --global, -g     Install globally (npm install -g kryos)"
...
print_banner() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${BOLD}Kryos${NC} — AI Agent Orchestration for Claude Code     ${CYAN}║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
verify_installation() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        VERSION_OUTPUT=$(kryos --version 2>/dev/null || claude-flow --version 2>/dev/null || echo "")
        if [ -z "$VERSION_OUTPUT" ]; then
            print_warning "Global command not found in PATH"
            print_substep "Try: ${BOLD}npm install -g kryos@${VERSION}${NC}"
...
show_quickstart() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        ...
        echo -e "  ${BOLD}kryos init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}kryos doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add kryos -- kryos mcp start${NC}"
    else
        echo -e "  ${DIM}# Initialize project${NC}"
        echo -e "  ${BOLD}npx kryos@latest init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}npx kryos@latest doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add kryos -- npx -y kryos@latest mcp start${NC}"
    fi

    echo ""
    echo -e "${DIM}Documentation: https://github.com/ruvnet/kryos${NC}"
    echo -e "${DIM}Issues: https://github.com/ruvnet/kryos/issues${NC}"
    echo ""
}
...
setup_mcp_server() {
    ...
    if claude mcp list 2>/dev/null | grep -q "kryos\|claude-flow"; then
    ...
    if [ "$GLOBAL" = "1" ]; then
        claude mcp add kryos -e CLAUDE_FLOW_CWD="$HOME" -- kryos mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add kryos -e CLAUDE_FLOW_CWD=\"\$HOME\" -- kryos mcp start"
    else
        claude mcp add kryos -e CLAUDE_FLOW_CWD="$HOME" -- npx -y kryos@${VERSION} mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add kryos -e CLAUDE_FLOW_CWD=\"\$HOME\" -- npx -y kryos@latest mcp start"
    fi
    echo ""
}
...
run_doctor() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        kryos doctor 2>&1 || true
    else
        npx kryos@${VERSION} doctor 2>&1 || true
    fi
    echo ""
}
...
run_init() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        kryos init --yes 2>&1 || true
    else
        npx kryos@${VERSION} init --yes 2>&1 || true
    fi
    echo ""
}
...
main() {
    ...
    print_success "${BOLD}Kryos is ready!${NC}"
    echo ""
}
