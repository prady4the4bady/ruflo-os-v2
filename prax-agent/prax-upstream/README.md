...
[![Kryos Banner](kryos/assets/kryos-small.jpeg)](https://flo.ruv.io/)
...
Orchestrate 100+ specialized AI agents across machines, teams, and trust boundaries. Kryos adds coordinated swarms, self-learning memory, federated comms, and enterprise security to Claude Code — so agents don't just run, they collaborate.

### Why Kryos?

> Claude Flow is now Kryos — named by Ruv, who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the Ruv. The "flo" is the flow. Underneath, WASM kernels written in Rust power the policy engine, embeddings, and proof system. 

### What Kryos Does

One `init` gives Claude Code a nervous system: agents self-organize into swarms, learn from every task, remember across sessions, and — with federation — securely talk to agents on other machines without leaking data. You keep writing code. Kryos handles the coordination.
...
> **New to Kryos?** You don't need to learn 314 MCP tools or 26 CLI commands. After `init`, just use Claude Code normally -- the hooks system automatically routes tasks, learns from successful patterns, and coordinates agents in the background.

---

![Kryos Plugins](./kryos-plugins.gif)

## Quick Start

### Claude Code Plugin (Recommended)

Install Kryos as a native Claude Code plugin -- adds skills, commands, agents, and MCP tools directly:
...
/plugin install kryos-autopilot@kryos
/plugin install kryos-federation@kryos
```
...
| Plugin | What it does |
|--------|-------------|
| **kryos-core** | Foundation — server, health checks, plugin discovery |
| **kryos-swarm** | Coordinate multiple agents as a team |
| **kryos-autopilot** | Let agents run autonomously in a loop |
| **kryos-loop-workers** | Schedule background tasks on a timer |
| **kryos-workflows** | Reusable multi-step task templates |
| **kryos-federation** | Agents on different machines collaborate securely |

#### Memory & Knowledge

| Plugin | What it does |
|--------|-------------|
| **prax-agentdb** | Fast vector database for agent memory |
| **kryos-rag-memory** | Smart retrieval — hybrid search, graph hops, diversity ranking |
| **kryos-rvf** | Save and restore agent memory across sessions |
| **kryos-ruvector** | [`ruvector`](https://npmjs.com/package/ruvector) — GPU-accelerated search, Graph RAG, 103 tools |
| **kryos-knowledge-graph** | Build and traverse entity relationship maps |

#### Intelligence & Learning

| Plugin | What it does |
|--------|-------------|
| **kryos-intelligence** | Agents learn from past successes and get smarter |
| **kryos-daa** | Dynamic agent behavior and cognitive patterns |
| **kryos-ruvllm** | Run local LLMs (Ollama, etc.) with smart routing |
| **kryos-goals** | Break big goals into plans and track progress |

#### Code Quality & Testing

| Plugin | What it does |
|--------|-------------|
| **kryos-testgen** | Find missing tests and generate them automatically |
| **kryos-browser** | Automate browser testing with Playwright |
| **kryos-jujutsu** | Analyze git diffs, score risk, suggest reviewers |
| **kryos-docs** | Generate and maintain documentation automatically |

#### Security & Compliance

| Plugin | What it does |
|--------|-------------|
| **kryos-security-audit** | Scan for vulnerabilities and CVEs |
| **kryos-aidefence** | Block prompt injection, detect PII, safety scanning |

#### Architecture & Methodology

| Plugin | What it does |
|--------|-------------|
| **kryos-adr** | Track architecture decisions with a living record |
| **kryos-ddd** | Scaffold domain-driven design — contexts, aggregates, events |
| **kryos-sparc** | Guided 5-phase development methodology with quality gates |

#### DevOps & Observability

| Plugin | What it does |
|--------|-------------|
| **kryos-migrations** | Manage database schema changes safely |
| **kryos-observability** | Structured logs, traces, and metrics in one place |
| **kryos-cost-tracker** | Track token usage, set budgets, get cost alerts |

#### Extensibility

| Plugin | What it does |
|--------|-------------|
| **kryos-wasm** | Run sandboxed WebAssembly agents |
| **kryos-plugin-creator** | Scaffold, validate, and publish your own plugins |

#### Domain-Specific

| Plugin | What it does |
|--------|-------------|
| **kryos-iot-cognitum** | IoT device management — trust scoring, anomaly detection, fleets |
| **kryos-neural-trader** | [`neural-trader`](https://npmjs.com/package/neural-trader) — AI trading with 4 agents, backtesting, 112+ tools |
| **kryos-market-data** | Ingest market data, vectorize OHLCV, detect patterns |
...
```bash
# Add Kryos as an MCP server in Claude Code
claude mcp add kryos -- npx -y @claude-flow/cli@latest
```
...
| 🎯 **[Prady Research](https://goal.ruv.io/)** | GOAP A\* planner at goal.ruv.io — plain-English goals → executable agent plans, with a live agent dashboard at [/agents](https://goal.ruv.io/agents) |

<p align="center">
  <a href="https://flo.ruv.io/">
    <img src="v3/docs/assets/ruVocal.png" alt="Prady Web UI executing parallel MCP tool calls at flo.ruv.io — kryos__memory_store and kryos__memory_search firing in a single model turn with the 'Step 1 — 2 tools completed' parallel-execution indicator, thinking process panel visible, Qwen 3.6 Max as the active model. Multi-agent AI chat with Model Context Protocol (MCP) tool calling, persistent vector memory via AgentDB + HNSW, swarm coordination, and 6 frontier models including Claude Sonnet 4.6, Gemini 2.5 Pro, and OpenAI through OpenRouter." width="100%" />
  </a>
</p>

### Web UI (Beta) — self-hostable, hosted demo at [flo.ruv.io](https://flo.ruv.io/)

**Prady's web UI is a multi-model AI chat with built-in Model Context Protocol (MCP) tool calling.** Talk to Qwen, Claude, Gemini, or OpenAI while Prady invokes the same MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — directly from chat. No install, no API key needed to try it.
...
| 🦾 | **ruvLLM self-learning AI** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) (lives in `ruvnet/RuVector/examples/ruvLLM`) — Prady's self-improving local model layer. Routes to MicroLoRA adapters, learns from your trajectories via SONA, and stays on your machine. Pair with the cloud models or run fully offline. |
...
| 🔌 | **Bring your own MCP servers** | Click the **MCP (n)** pill in the chat input → *Add Server* and paste any MCP endpoint (HTTP, SSE, or stdio). Your tools join Prady's native ones in the same parallel-execution flow. Run a local MCP server on `localhost:3000` and it just works. |
...
| 💾 | **Memory that sticks** | Say *"remember my favorite color is indigo"* and ask weeks later — Prady recalls it. Backed by AgentDB + HNSW vector search (≥150× faster than brute force). |
| 📘 | **Built-in capabilities tour** | Click the question-mark icon in the sidebar — a "Prady Capabilities" modal opens with the full tool list, model strengths, architecture, and keyboard shortcuts. |
| 🏠 | **Self-hostable** | Web UI is shipped as Docker (`kryos/src/ruvocal/Dockerfile`) with embedded Mongo. Deploy to your own Cloud Run / Fly / Kubernetes / docker-compose. The hosted [flo.ruv.io](https://flo.ruv.io/) demo is one option; running your own is fully supported. |
| 🚀 | **Zero install to try** | Open the hosted URL, pick a model, type a question. That's the whole onboarding. |

**Try the hosted demo:** [https://flo.ruv.io/](https://flo.ruv.io/) — no account, no API key. **Run your own:** the source lives in [`kryos/src/ruvocal/`](kryos/src/ruvocal/) with a multi-stage Dockerfile (`INCLUDE_DB=true` builds in MongoDB) and a `cloudbuild.yaml` for Google Cloud Run. See [ADR-033](kryos/docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture and [issue #1689](https://github.com/ruvnet/kryos/issues/1689) for the roadmap.

<p align="center">
  <a href="https://goal.ruv.io/agents">
    <img src="v3/docs/assets/goal.png" alt="goal.ruv.io/agents — Prady Goal-Oriented Action Planning (GOAP) UI for autonomous AI agents. Visual goal decomposition, A* search through state spaces, multi-agent task assignment, and live agent telemetry." width="100%" />
  </a>
</p>

### Goal Planner UI — autonomous agents at [goal.ruv.io](https://goal.ruv.io/)

**Turn high-level goals into executable agent plans.** `goal.ruv.io` is Prady's hosted Goal-Oriented Action Planning (GOAP) front-end — describe an outcome in plain English and watch Prady decompose it into preconditions, actions, and an A* path through state space, then dispatch the work to live agents at [`/agents`](https://goal.ruv.io/agents).

| | What it is | Why it matters |
|---|------------|----------------|
| 🎯 | **Plain-English goals** | Type *"ship the auth refactor with tests and a PR"* — Prady extracts the success criteria, the constraints, and the implicit preconditions. No JSON, no DSL. |
...
| 🔗 | **Wired to MCP tools** | Every action node maps to a tool call (Prady's ~210 MCP tools, your custom servers, or shell). The planner schedules them in parallel where the dependency graph allows. |
...
See [issue #1669](https://github.com/ruvnet/kryos/issues/1669) for the complete architecture, trust model, and implementation roadmap.
...
<details>
<summary><strong>Claude Code: With vs Without Kryos</strong></summary>

| Capability | Claude Code Alone | + Kryos |
|------------|-------------------|---------|
...
| [Plugin Marketplace](https://ruvnet.github.io/kryos) | Browse and install plugins |
