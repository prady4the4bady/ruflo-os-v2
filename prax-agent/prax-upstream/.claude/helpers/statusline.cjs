#!/usr/bin/env node
/**
 * Prady V3 Statusline Generator (Optimized)
...
function generateStatusline() {
  ...
  // Branding
  parts.push(`${c.bold}${c.brightPurple}\u258A Prady v${pkgVersion || '3.5'}${c.reset}`);
...
function generateDashboard() {
  ...
  // Header
  let header = `${c.bold}${c.brightPurple}\u258A Prady v${pkgVersion} ${c.reset}`;
