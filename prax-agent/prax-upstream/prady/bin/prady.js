#!/usr/bin/env node
// Kryos CLI - thin wrapper around @claude-flow/cli with kryos branding
...
// Walk up from kryos/bin/ to find @claude-flow/cli in node_modules
...
if (isMCPMode) {
  ...
  // CLI mode: use kryos branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'kryos',
    description: 'Kryos - AI Agent Orchestration Platform',
  });
  cli.run()
    .then(() => {
