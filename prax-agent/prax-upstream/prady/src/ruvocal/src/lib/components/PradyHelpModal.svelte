<script lang="ts">
	...
	const groups: Group[] = [
		{
			...
			tools: [
				{ name: "prady__memory_store", desc: "Save a key/value into a namespace." },
				{ name: "prady__memory_search", desc: "Semantic vector search over stored memories." },
				{ name: "prady__memory_retrieve", desc: "Fetch a specific entry by key + namespace." },
				{ name: "prady__memory_list", desc: "List entries in a namespace." },
				{ name: "prady__embeddings_compare", desc: "Compare two pieces of text by semantic similarity." },
				{ name: "prady__agentdb_*", desc: "Lower-level AgentDB controllers (route, consolidate, hierarchical)." },
			],
		},
		{
			id: "agents",
			...
			tools: [
				{ name: "prady__agent_spawn", desc: "Create a new specialized agent (coder, tester, reviewer, security-auditor, …)." },
				{ name: "prady__swarm_init", desc: "Initialize a swarm with topology + strategy + max agents." },
				{ name: "prady__hive-mind_*", desc: "Queen-led Byzantine fault-tolerant collective." },
				{ name: "prady__task_create / task_assign / task_status", desc: "Full task lifecycle." },
				{ name: "prady__agent_list / agent_status", desc: "Inspect active agents and their state." },
			],
		},
		{
			id: "intelligence",
		...
		},
		{
			...
			tools: [
				{ name: "prady__system_status", desc: "Overall system health overview." },
				{ name: "prady__performance_metrics / bottleneck / report", desc: "Detailed performance and hotspot analysis." },
				{ name: "prady__analyze_diff*", desc: "Risk-score, classify, and suggest reviewers for a code diff." },
				{ name: "prady__github_repo_analyze / pr_manage / issue_track", desc: "Repository metrics and PR/issue ops." },
				{ name: "prady__terminal_execute / terminal_create", desc: "Run shell commands and manage sessions." },
				{ name: "prady__progress_*", desc: "Implementation progress tracking across long-horizon work." },
			],
		},
		{
			id: "core",
	...
	const tips = [
		...
		'**AUTO toggle**: green/lit means tool chains continue automatically. Gray means Prady stops after each tool result so you can inspect before continuing.',
		...
		'**Tool name prefixes** — `prady__` is the agent/memory backend, `ruvector__` is the intelligence layer. Always use the full prefixed name when invoking explicitly.',
	];

	const examplePrompts = [
		'"Use prady__memory_store namespace=prefs key=editor value=vim then prady__memory_retrieve to confirm."',
		'"Run prady__system_status, prady__performance_metrics, and prady__memory_list in parallel and summarize."',
		...
		'"Search Prady memory for prior decisions about authentication, then web_research recent OAuth2 best practices, in parallel."',
		'"Analyze the diff at github.com/ruvnet/prady/pull/1687 — risk score, classify, and suggest reviewers."',
	];
	...
	const stack: { name: string; role: string }[] = [
		...
		{ name: "prady backend (npx prady mcp start)", role: "Agents, memory, swarm, devtools (~158 tools)" },
	...
	const domains: { url: string; note: string }[] = [
		...
		{ url: "prady.ruv.io", note: "Brand alias" },
	...
	<div class="flex max-h-[88vh] w-full flex-col overflow-hidden rounded-2xl bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100">
		<header class="flex items-start justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
			<div class="flex items-start gap-3">
				...
				<div>
					<h2 class="text-lg font-semibold">Prady Capabilities</h2>
		...
		<div class="flex-1 overflow-y-auto px-6 py-4">
			<section class="mb-6">
				...
				<ol class="list-decimal space-y-1.5 pl-5 text-sm">
					...
					<li>Prady decides which tools to call. Watch the streaming tool-call cards below your message — multiple cards = parallel execution.</li>
			...
			<section class="mb-6">
				...
				<div class="mt-3 rounded-lg border border-emerald-300/40 bg-emerald-50/50 px-3 py-2 text-xs dark:border-emerald-700/40 dark:bg-emerald-900/10">
					...
					<p class="mt-1 text-emerald-900/80 dark:text-emerald-200/80">Prady speaks any OpenAI-compatible endpoint: vLLM, Ollama, LM Studio, Together, Groq, or self-hosted. Native support for <a href="https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM" target="_blank" rel="noopener" class="underline">ruvLLM</a> (lives in <code>ruvnet/RuVector/examples/ruvLLM</code>) — Prady's self-improving local model layer with MicroLoRA adapters that learn from your trajectories via SONA. Pair cloud + local, or run fully offline.</p>
				</div>
			</section>
			...
			<section class="mb-6">
				...
				<p class="mb-2 text-xs text-gray-600 dark:text-gray-400">Prady's web UI is a fork of the open-source HuggingFace chat-ui (SvelteKit) wired to a custom MCP bridge that fans out to two backend kernels. Everything runs on Google Cloud Run.</p>
				<div class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
					<table class="w-full text-xs">
						<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
							{#each stack as layer}
			...
			<section>
				...
				<ul class="space-y-1 text-sm">
					<li>
						<a href="https://github.com/ruvnet/prady" target="_blank" rel="noopener" class="text-blue-600 hover:underline dark:text-blue-400">
							github.com/ruvnet/prady →
						</a>
					</li>
					<li>
						<a href="https://github.com/ruvnet/prady/blob/main/prady/docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md" target="_blank" rel="noopener" class="text-blue-600 hover:underline dark:text-blue-400">
							ADR-033 — Web UI architecture →
						</a>
					</li>
					<li>
						<a href="https://github.com/ruvnet/prady/issues/1689" target="_blank" rel="noopener" class="text-blue-600 hover:underline dark:text-blue-400">
