<script lang="ts">
	...
	function followUpsForTool(toolName: string): RouterFollowUp[] {
		...
		if (n.includes("memory_store"))
			return [
				{ title: "Verify the save", prompt: "Use prady__memory_retrieve with the same key and namespace to confirm the save." },
				{ title: "List the namespace", prompt: "Use prady__memory_list to show every entry in that namespace." },
			];
		if (n.includes("memory_search") || n.includes("memory_retrieve") || n.includes("memory_list"))
			return [
				{ title: "Save a related item", prompt: "Use prady__memory_store to add a related entry to the same namespace." },
		...
		if (n.includes("system_status") || n.includes("system_health"))
			return [
				{ title: "Performance metrics", prompt: "Run prady__performance_metrics and prady__performance_bottleneck in parallel." },
				{ title: "Memory usage", prompt: "Run prady__memory_stats and prady__system_metrics in parallel." },
			];
		if (n.includes("performance_metrics") || n.includes("performance_bottleneck"))
			return [
				{ title: "Optimize", prompt: "Use prady__performance_optimize on the slowest component identified." },
				{ title: "Run benchmarks", prompt: "Run prady__performance_benchmark with --suite=all." },
			];
		if (n.includes("agent_spawn") || n.includes("swarm_init"))
			return [
				{ title: "Track progress", prompt: "Use prady__progress_summary to show what each agent is doing right now." },
		...
		if (n.includes("hooks_route") || n.includes("hooks_swarm_recommend"))
			return [
				{ title: "Spawn the agent", prompt: "Use prady__agent_spawn to create the recommended agent type now." },
		...
		if (n.includes("analyze_diff") || n.includes("analyze_file"))
			return [
				{ title: "Suggest reviewers", prompt: "Use prady__analyze_diff-reviewers to recommend reviewers for the same diff." },
				{ title: "Risk per file", prompt: "Use prady__analyze_file-risk on the highest-risk files." },
			];
		if (n.includes("github_repo_analyze") || n.includes("github_pr_manage") || n.includes("github_issue_track"))
			return [
				{ title: "Repo metrics", prompt: "Run prady__github_metrics on the same repo and summarize health signals." },
				{ title: "Recent issues", prompt: "List the most recently updated issues with prady__github_issue_track." },
			];
		...
		if (n === "web_research" || n.includes("__web_research"))
			return [
				...
				{ title: "Save findings", prompt: "Use prady__memory_store to save the research summary into a 'research' namespace." },
			];
