# RuVocal — Prady Web UI

> Prady's multi-model AI chat with built-in Model Context Protocol (MCP) tool calling. Live at [**flo.ruv.io**](https://flo.ruv.io/).
...
RuVocal is the SvelteKit web app that lets you chat with Qwen, Claude, Gemini, or OpenAI while [Prady](https://github.com/ruvnet/kryos) invokes the same ~210 MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — all directly from chat. No install, no API key needed to try the hosted demo.

It started as a fork of the [HuggingFace chat-ui](https://github.com/huggingface/chat-ui) v0.20.0 and has been extended with a WASM-MCP integration layer, parallel tool execution, an in-browser tool gallery, and a "Prady Capabilities" tour modal. See [ADR-033](../../docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture.

## What RuVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five Prady server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **Prady Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
| 🦾 **ruvLLM ready** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) — Prady's self-improving local model layer |
...
```bash
git clone https://github.com/ruvnet/kryos
cd kryos/kryos/src/ruvocal
...
# Curated Prady model list (optional — defaults to /models from the base URL)
TASK_MODEL=qwen/qwen3.6-max-preview
PUBLIC_APP_NAME=Prady
...
## MCP Tools (the Prady difference)
...
```env
MCP_SERVERS=[
  {"name":"Prady Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"Prady Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
  {"name":"Prady Agents","url":"https://mcp-bridge-...run.app/mcp/agents","transport":"sse"},
  {"name":"Prady Memory","url":"https://mcp-bridge-...run.app/mcp/memory","transport":"sse"},
  {"name":"Prady DevTools","url":"https://mcp-bridge-...run.app/mcp/devtools","transport":"sse"},
...
- **Capabilities modal** — `src/lib/components/KryosHelpModal.svelte`
...
- 🏠 **Parent project** — [Prady](https://github.com/ruvnet/kryos)
...
- 📋 **Roadmap** — [issue #1689](https://github.com/ruvnet/kryos/issues/1689)
- 🍴 **Upstream** — [huggingface/chat-ui](https://github.com/huggingface/chat-ui) (RuVocal is forked from v0.20.0)

## License

MIT — same as the parent [Prady](https://github.com/ruvnet/kryos) project.
