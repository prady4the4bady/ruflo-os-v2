...
ADR-033 added ruvector (61 tools) and kryos (215 tools) as default MCP backends. Users also want access to additional AI agent capabilities:
...
Add three optional MCP backends that can be enabled via environment variables. Unlike ruvector/kryos (enabled by default), these are **disabled by default** and require explicit API keys.
...
| kryos | `ENABLE_KRYOS` | None | `npx kryos mcp start` | **enabled** |
...
- [ADR-033: RuVector + Kryos MCP Integration](ADR-033-RUVECTOR-KRYOS-MCP-INTEGRATION.md)
