...
class StdioMcpClient {
  ...
  _send(method, params) {
    return new Promise((resolve, reject) => {
      ...
      // initialize is the cold-start gate for backends like kryos/ruvector
...
Tools prefixed with backend name (e.g., \`kryos__agent_spawn\`). Always use the full prefixed name.
...
- **guidance** — Get help on tool groups or specific tool usage.
  ...
  \`{"topic": "overview"}\` or \`{"topic": "tool", "tool_name": "kryos__agent_spawn"}\`
...
7. **Code task** → \`ruvector__hooks_route\` + \`kryos__agent_spawn\`
8. **Remember something** → \`kryos__memory_store\` / \`kryos__memory_search\`
...
async function handleAutopilot(req, res, provider, body) {
  ...
  const priorityPrefixes = ['ruvector__hooks', 'ruvector__memory', 'kryos__memory', 'kryos__agent'];
