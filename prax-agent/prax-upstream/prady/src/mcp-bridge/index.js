...
const TOOL_GROUPS = {
  ...
  // --- Agents & Orchestration (kryos) ---
  agents: {
    enabled: process.env.MCP_GROUP_AGENTS !== "false",
    description: "Agent lifecycle, swarm coordination, task management, workflows (kryos)",
    source: "kryos",
    prefixes: ["agent_", "swarm_", "task_", "session_", "hive-mind_", "workflow_", "coordination_"],
  },

  // --- Memory & Knowledge (kryos) ---
  memory: {
    enabled: process.env.MCP_GROUP_MEMORY !== "false",
    description: "Vector memory, AgentDB, embeddings, semantic search (kryos)",
    source: "kryos",
  ...
  devtools: {
    ...
    description: "Hooks, code analysis, performance profiling, GitHub integration (kryos)",
    source: "kryos",
    prefixes: ["hooks_", "analyze_", "performance_", "github_", "terminal_", "config_", "system_", "progress_"],
  },

  // --- Security & Safety (kryos) ---
  security: {
    enabled: process.env.MCP_GROUP_SECURITY === "true",
    description: "AI defence, PII detection, claims management, pattern transfer (kryos)",
    source: "kryos",
    prefixes: ["aidefence_", "claims_", "transfer_"],
  },

  // --- Browser Automation (kryos) ---
  browser: {
    enabled: process.env.MCP_GROUP_BROWSER === "true",
    description: "Headless browser control — navigate, click, fill, screenshot (kryos)",
    source: "kryos",
    prefixes: ["browser_"],
  },
  ...
  neural: {
    ...
    description: "Neural network training, DAA autonomous agents, cognitive patterns (kryos)",
    source: "kryos",
...
const BACKEND_DEFS = [
  ...
  { name: "kryos",          command: "npx", args: ["-y", "kryos", "mcp", "start"],      groups: ["agents", "memory", "devtools", "security", "browser", "neural"] },
...
- To recall previously stored patterns or solutions`,
    ...
    agents: `# Agents & Orchestration Group (kryos)

Multi-agent lifecycle management, swarm coordination, and task workflows.

## Key Tools
- **kryos__agent_spawn** — Create a new agent with specific capabilities
- **kryos__agent_list** — List all active agents
- **kryos__swarm_init** — Initialize a swarm with a topology (mesh, hierarchical, ring, star)
- **kryos__task_create** — Create and assign tasks
- **kryos__workflow_create** — Define multi-step workflows
- **kryos__workflow_execute** — Execute a workflow
- **kryos__hive-mind_init** — Start collective intelligence coordination
- **kryos__coordination_orchestrate** — Multi-agent coordination
...
- Distributed task management`,
    ...
    memory: `# Memory & Knowledge Group (kryos)

Vector storage, semantic search, AgentDB pattern learning, and embeddings.

## Key Tools
- **kryos__memory_store** — Store a value with vector embedding for semantic search
- **kryos__memory_search** — Semantic search across stored memories (HNSW-indexed)
- **kryos__memory_list** — List stored memory entries
- **kryos__agentdb_pattern-store** — Store a reasoning pattern for learning
- **kryos__agentdb_pattern-search** — Search for similar reasoning patterns
- **kryos__agentdb_context-synthesize** — Synthesize context from stored memories
- **kryos__embeddings_generate** — Generate vector embeddings for text
- **kryos__embeddings_search** — Semantic similarity search
...
- Building semantic search over custom data`,
    ...
    devtools: `# Dev Tools Group (kryos)

Code analysis, performance profiling, GitHub integration, and terminal access.

## Key Tools
- **kryos__analyze_diff** — Analyze git diff for risk and change classification
- **kryos__performance_benchmark** — Run performance benchmarks
- **kryos__performance_bottleneck** — Detect performance bottlenecks
- **kryos__github_repo_analyze** — Analyze a GitHub repository
- **kryos__github_pr_manage** — Manage pull requests
- **kryos__terminal_execute** — Execute commands in a terminal session
...
- GitHub repository management`,
    ...
    security: `# Security & Safety Group (kryos)

AI defence, PII detection, and claims-based authorization.

## Key Tools
- **kryos__aidefence_scan** — Scan text for AI manipulation attempts
- **kryos__aidefence_has_pii** — Check for PII (emails, phones, SSNs)
- **kryos__aidefence_is_safe** — Quick safety check on input
- **kryos__claims_claim** — Claim an issue for work
- **kryos__claims_board** — Visual board of all claims
...
- Work item management across agents`,
    ...
    browser: `# Browser Automation Group (kryos)

Headless browser control for web interaction and testing.

## Key Tools
- **kryos__browser_open** — Navigate to a URL
- **kryos__browser_click** — Click elements by reference
- **kryos__browser_fill** — Fill form inputs
- **kryos__browser_screenshot** — Capture page screenshots
- **kryos__browser_snapshot** — Get accessibility tree for AI parsing
- **kryos__browser_eval** — Execute JavaScript in page context
...
- Form filling and web interaction`,
    ...
    neural: `# Neural & DAA Group (kryos)

Neural network operations and Decentralized Autonomous Agents.

## Key Tools
- **kryos__neural_train** — Train a neural model
- **kryos__neural_predict** — Make predictions
- **kryos__daa_agent_create** — Create an autonomous agent
- **kryos__daa_workflow_create** — Create autonomous workflows
- **kryos__daa_knowledge_share** — Share knowledge between agents
...
Your tools are organized into groups. Each tool name is prefixed with its backend (e.g., \`kryos__agent_spawn\`, \`ruvector__hooks_route\`). Always use the full prefixed name when calling tools.
...
- **guidance** — Get help on any tool group, specific tool usage, or capabilities overview.
  ...
  For specific tool help: \`guidance(topic='tool', tool_name='kryos__agent_spawn')\`
...
- **kryos__agent_status** — Check an agent's current state. \`{"agentId": "agent-xxx"}\`
- **kryos__agent_list** — List all active agents with their states.
- **kryos__agent_terminate** — Stop an agent. \`{"agentId": "agent-xxx"}\`
- **kryos__agent_health** — Health check across all agents.
- **kryos__agent_pool** — View the agent pool and available capacity.

### Swarm Coordination:
- **kryos__swarm_init** — Initialize a multi-agent swarm.
...
- **kryos__swarm_status** — Get swarm health, topology, and agent states.
- **kryos__swarm_health** — Detailed health metrics for the swarm.
- **kryos__swarm_shutdown** — Tear down a swarm and all its agents.

### Task Management:
- **kryos__task_create** — Create a tracked task.
  \`{"description": "what needs to be done", "priority": "low|normal|high|critical"}\`
- **kryos__task_status** — Check task progress. \`{"taskId": "task-xxx"}\`
- **kryos__task_list** — List all tasks with their statuses.
- **kryos__task_complete** — Mark a task as done. \`{"taskId": "task-xxx"}\`
- **kryos__task_update** — Update task details, status, or assignment.
- **kryos__task_cancel** — Cancel a task.

### Workflow Orchestration:
- **kryos__workflow_create** — Define a multi-step workflow with dependencies.
- **kryos__workflow_execute** — Run a workflow. \`{"workflowId": "wf-xxx"}\`
- **kryos__workflow_status** — Check workflow progress.
- **kryos__workflow_template** — Use a pre-built workflow template.
- **kryos__workflow_pause** / **kryos__workflow_resume** — Control workflow execution.

### Hive-Mind (Distributed Consensus):
- **kryos__hive-mind_init** — Start distributed consensus system.
- **kryos__hive-mind_spawn** — Add an agent to the hive.
- **kryos__hive-mind_consensus** — Run consensus vote across agents.
- **kryos__hive-mind_broadcast** — Send message to all hive agents.
- **kryos__hive-mind_memory** — Access shared hive memory.

### Coordination:
- **kryos__coordination_topology** — View/change coordination topology.
- **kryos__coordination_load_balance** — Distribute work across agents.
- **kryos__coordination_orchestrate** — Orchestrate complex multi-agent tasks.
- **kryos__coordination_sync** — Synchronize state across agents.

### Session Management:
- **kryos__session_save** — Save current session state.
- **kryos__session_restore** — Restore a previous session.
- **kryos__session_list** — List available sessions.

## Group 4: Memory & Knowledge (kryos)
Persistent memory, vector search, embeddings, and pattern storage. ${TOOL_GROUPS.memory.enabled ? "ACTIVE" : "DISABLED"}

### Memory Operations:
- **kryos__memory_store** — Store data in persistent memory.
  \`{"key": "my-key", "value": "data to store", "namespace": "default", "tags": ["tag1"]}\`
- **kryos__memory_retrieve** — Get stored data by key. \`{"key": "my-key"}\`
- **kryos__memory_search** — Semantic vector search across stored memories.
  \`{"query": "what to search for", "limit": 5, "namespace": "default"}\`
- **kryos__memory_list** — List all stored keys in a namespace.
- **kryos__memory_delete** — Remove a stored memory.
- **kryos__memory_stats** — View memory usage statistics.

### Embeddings:
- **kryos__embeddings_generate** — Generate vector embeddings for text.
- **kryos__embeddings_compare** — Compare semantic similarity of two texts.
- **kryos__embeddings_search** — Search embeddings database by similarity.
- **kryos__embeddings_neural** — Generate neural embeddings.
- **kryos__embeddings_hyperbolic** — Generate hyperbolic embeddings for hierarchical data.

### AgentDB (Advanced Pattern Storage):
- **kryos__agentdb_pattern-store** — Store a learned pattern with metadata.
  \`{"pattern": "description", "category": "code|debug|architecture", "confidence": 0.9}\`
- **kryos__agentdb_pattern-search** — Search patterns by similarity.
- **kryos__agentdb_route** — Route a query to the most relevant stored pattern.
- **kryos__agentdb_feedback** — Provide feedback on a pattern (reinforcement learning).
- **kryos__agentdb_context-synthesize** — Synthesize context from multiple sources.
- **kryos__agentdb_semantic-route** — Semantic routing based on stored knowledge.
- **kryos__agentdb_consolidate** — Consolidate and deduplicate stored patterns.
- **kryos__agentdb_batch** — Batch operations on patterns.
- **kryos__agentdb_session-start** / **kryos__agentdb_session-end** — Session tracking.
- **kryos__agentdb_hierarchical-store** / **kryos__agentdb_hierarchical-recall** — Hierarchical memory.

## Group 5: Dev Tools & Analysis (kryos)
Performance, system health, GitHub integration, code analysis, terminal. ${TOOL_GROUPS.devtools.enabled ? "ACTIVE" : "DISABLED"}

### System & Performance:
- **kryos__system_status** — System health overview.
- **kryos__system_metrics** — Detailed performance metrics.
- **kryos__system_health** — Health check across all subsystems.
- **kryos__performance_report** — Generate performance report.
- **kryos__performance_bottleneck** — Identify performance bottlenecks.
- **kryos__performance_benchmark** — Run benchmarks.
- **kryos__performance_optimize** — Get optimization recommendations.
- **kryos__performance_profile** — Profile specific operations.

### Code Analysis:
- **kryos__analyze_diff** — Analyze a code diff.
- **kryos__analyze_diff-risk** — Assess risk level of changes.
- **kryos__analyze_diff-classify** — Classify type of changes (feature, bugfix, refactor).
- **kryos__analyze_diff-reviewers** — Suggest code reviewers.
- **kryos__analyze_file-risk** — Assess risk of a specific file.

### GitHub Integration:
- **kryos__github_repo_analyze** — Analyze a GitHub repository.
  \`{"repo": "owner/repo", "analysis_type": "code_quality|performance|security"}\`
- **kryos__github_pr_manage** — Manage pull requests (create, review, merge).
- **kryos__github_issue_track** — Track and manage issues.
- **kryos__github_workflow** — Manage GitHub Actions workflows.
- **kryos__github_metrics** — Repository metrics and insights.

### Terminal Access:
- **kryos__terminal_create** — Create a terminal session.
- **kryos__terminal_execute** — Execute a command. \`{"command": "ls -la"}\`
- **kryos__terminal_list** — List active terminals.
- **kryos__terminal_history** — View command history.

### Development Hooks:
- **kryos__hooks_pre-task** / **kryos__hooks_post-task** — Task lifecycle hooks for learning.
- **kryos__hooks_pre-edit** / **kryos__hooks_post-edit** — File edit hooks.
- **kryos__hooks_session-start** / **kryos__hooks_session-end** — Session lifecycle.
- **kryos__hooks_worker-dispatch** — Dispatch background workers.
  Workers: \`optimize\`, \`audit\`, \`testgaps\`, \`document\`, \`map\`, \`deepdive\`, \`benchmark\`
- **kryos__hooks_model-route** — Route to optimal AI model for a task.
- **kryos__hooks_explain** — Explain a routing or intelligence decision.

### Configuration:
- **kryos__config_get** / **kryos__config_set** / **kryos__config_list** — Manage settings.

### Progress Tracking:
- **kryos__progress_check** — Check implementation progress.
- **kryos__progress_summary** — Summarize overall progress.
...
3. **Code task** ("write", "fix", "implement") → \`ruvector__hooks_route\` to find best approach, then \`kryos__agent_spawn\`
4. **Analysis request** ("analyze", "review", "audit") → spawn reviewer/security agents + analysis tools
5. **Multi-step project** → \`kryos__task_create\` for tracking, \`kryos__swarm_init\` for coordination
6. **Memory/recall** ("remember", "save", "what did we...") → \`kryos__memory_store\` / \`kryos__memory_search\`
7. **System question** ("what tools", "help") → \`guidance(topic='overview')\`
8. **Performance concern** → \`kryos__performance_bottleneck\` + \`kryos__performance_optimize\`
9. **GitHub task** → \`kryos__github_*\` tools
...
### Code Implementation
\`ruvector__hooks_route\` → \`kryos__agent_spawn(coder)\` → track with \`kryos__task_create\` → report

### Multi-Agent Analysis
\`kryos__swarm_init(hierarchical)\` → spawn agents → coordinate → synthesize results

### Learning & Memory
\`kryos__memory_search\` (check existing) → do work → \`kryos__memory_store\` (save results) → \`ruvector__hooks_learn\`
...
- The prefixes "kryos__" or "ruvector__" — just describe what you're doing naturally
