...
- Misc: `Modal.svelte`, `NavMenu.svelte`, `PradyUniverse.svelte`, `Switch.svelte`, `WelcomeModal.svelte`, `Logo.svelte`, `mcpExamples.ts`, `.gitignore`, `rvf.manifest.json`

**Local-only (must preserve):**
- `mcp-bridge/index.js` — local MCP bridge implementation (absent in upstream)
- `src/routes/api/v2/debug/` — debug routes used by kryos
- `stub/@reflink/reflink/index.js` — reflink stub
- `.env` — populated local environment
- `package-lock.json` — local lockfile
- All kryos-specific docs/CLAUDE.md guidance

`package.json` is identical between local and upstream — no dependency changes required.

## Decision

We will pull the upstream improvements into `kryos/src/ruvocal/` on a dedicated feature branch (`feat/ruvocal-wasm-mcp-integration`) using a **directory-level overlay strategy** rather than a Git merge, because the local snapshot has no shared history with the upstream repository.
...
3. **Verification** — `npm install`, `npm run check`, `npm run build`, then local docker compose + `kryos-browser` smoke test.
...
- No changes to `kryos/src/chat-ui/` (the thin HF base-image wrapper) in this ADR.
...
- Changes to `Settings.ts` / `Tool.ts` types could ripple into kryos packages that import from `src/ruvocal`. Mitigation: run `npm run check` before merging.
...
- [ ] `kryos-browser` smoke test: load the home page, open the gallery panel, send a message through a non-WASM model, confirm no console errors.
...
Cloud Run deployment is **out of scope for this PR** but the path is staged in `kryos/src/ruvocal/cloudbuild.yaml`. Two infrastructure prerequisites must be satisfied before the first deploy:
...
The thin `kryos/src/chat-ui/Dockerfile` wrapper (FROM `ghcr.io/huggingface/chat-ui-db:latest`) is **unsuitable** for deploying this integration — it can only patch the upstream HF base image with a few static files; it cannot include compiled WASM source. The full ruvocal Dockerfile build is required.
...
**Fix:** `kryos/src/ruvocal/.gcloudignore` (commit `e3b74f606`). All `/api/v2/*` routes now serve correctly. Verified via `gsutil cp` of the build's source tarball that all `+server.ts` files for the models subtree are present after the fix.
...
- **Help icon** in sidebar opens "Prady Capabilities" modal (`src/lib/components/KryosHelpModal.svelte`) with all 6 tool groups, quick-start, tips, and resource links.
- **8 Prady-themed example prompts** on the welcome screen (`src/lib/constants/{mcpExamples,routerExamples}.ts`) replacing HF defaults.
- **`PUBLIC_APP_NAME=Prady`** brands the title bar, sidebar, welcome modal, and PWA manifest.
...
## Follow-Up Work (tracked separately in [issue #1689](https://github.com/ruvnet/kryos/issues/1689))
...
| **P3** | **Help-modal copy fix** | Quick Start section says "default: Gemini 2.5 Flash". Update to "default: Claude Sonnet 4.6" in `src/lib/components/KryosHelpModal.svelte`. |
...
- Local target: `kryos/src/ruvocal/`
- Branch: `feat/ruvocal-wasm-mcp-integration` (squash-merged to `main`)
- PR: https://github.com/ruvnet/kryos/pull/1687
- Follow-up issue: https://github.com/ruvnet/kryos/issues/1689
- Cloud Build configs: `kryos/src/ruvocal/cloudbuild.yaml`, `kryos/src/ruvocal/mcp-bridge/cloudbuild.yaml`
- `.gcloudignore` (CRITICAL — see resolved root cause): `kryos/src/ruvocal/.gcloudignore`
- Web Worker: `kryos/src/ruvocal/src/lib/wasm/{wasm.worker.ts,workerClient.ts}`
- Help modal: `kryos/src/ruvocal/src/lib/components/KryosHelpModal.svelte`
- Live URLs: https://flo.ruv.io/ · https://kryos.ruv.io/ · https://ruvocal.ruv.io/
