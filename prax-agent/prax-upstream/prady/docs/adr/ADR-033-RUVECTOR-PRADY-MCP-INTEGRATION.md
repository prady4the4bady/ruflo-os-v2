# ADR-033: RuVector + Kryos MCP Tool Integration
...
The MCP bridge initially shipped with 3 built-in tools (search, web_research, system_guide). Users want access to the full ruvector (10 tools) and kryos (205+ tools) ecosystems from within the HF Chat UI without running separate MCP servers.
...
| **kryos** | 205+ | Agent (7), Swarm (4), Memory (7), Config (6), Hooks (40+), Task (6), Session (5), Hive-mind (9), Workflow (9), Analyze (4), Progress (4), AIDefence (6), AgentDB (14+) |

## Decision

Integrate ruvector and kryos as **stdio MCP child processes** spawned by the bridge, with tool calls proxied through the existing `/mcp` HTTP endpoint.
...
└─────────────────────────────────────────────────┘
        ...
        │                           │
   npx ruvector mcp start    npx kryos mcp start
```

### Key Design Decisions

1. **Namespaced tool names**: External tools are prefixed with `{backend}__` (e.g., `ruvector__hooks_route`, `kryos__agent_spawn`) to avoid name collisions with built-in tools.
...
3. **Environment toggle**: Each backend can be disabled via `ENABLE_RUVECTOR=false` or `ENABLE_KRYOS=false` for deployments that don't need all tools.
...
ENABLE_KRYOS=true       # default: true
```
