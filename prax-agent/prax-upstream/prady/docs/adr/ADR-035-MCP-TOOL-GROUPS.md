...
The MCP bridge grew to 331+ tools from multiple backends (ruvector, kryos, agentic-flow, Claude Code, Gemini, Codex). Exposing all tools simultaneously caused:
...
| **agents** | kryos | ~50 | enabled | `MCP_GROUP_AGENTS` |
| **memory** | kryos | ~25 | enabled | `MCP_GROUP_MEMORY` |
| **devtools** | kryos | ~60 | enabled | `MCP_GROUP_DEVTOOLS` |
| **security** | kryos | ~25 | disabled | `MCP_GROUP_SECURITY` |
| **browser** | kryos | ~23 | disabled | `MCP_GROUP_BROWSER` |
| **neural** | kryos | ~20 | disabled | `MCP_GROUP_NEURAL` |
...
// kryos tool "agent_spawn" → matches "agent_" prefix → included if agents group enabled
// kryos tool "browser_open" → matches "browser_" prefix → only if browser group enabled
```

A backend is only started if at least one of its groups is enabled. This means disabling all kryos groups prevents the kryos process from spawning entirely.
...
guidance(topic="tool", tool_name="kryos__memory_search") → specific tool docs
```
...
### Negative
- Some tools appear in multiple potential groups (e.g., kryos `hooks_*` in both intelligence and devtools) — resolved by prefix matching
- Group boundaries are somewhat arbitrary for the kryos "Uncategorized" tools
