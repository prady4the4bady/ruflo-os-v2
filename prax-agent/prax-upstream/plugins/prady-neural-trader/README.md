...
Wraps the `neural-trader` npm package as a Kryos plugin with 4 specialized agents, 6 skills, and comprehensive CLI commands. Adds AgentDB memory persistence, SONA trajectory learning, and swarm-coordinated execution on top of neural-trader's Rust/NAPI engine.
...
```bash
claude --plugin-dir plugins/kryos-neural-trader
```
...
- `kryos-market-data` — OHLCV data ingestion and candlestick pattern detection
- `kryos-ruvector` — HNSW indexing for strategy pattern similarity search
- `kryos-cost-tracker` — PnL tracking and cost attribution
- `kryos-observability` — Strategy performance dashboards
