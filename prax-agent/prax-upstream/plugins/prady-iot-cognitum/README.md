...
Treats every Cognitum Seed device as a Kryos agent with hardware capabilities. Devices progress through a 5-tier trust model, emit telemetry vectors for anomaly detection, participate in mesh networks, and maintain Ed25519 witness chains for provenance.
...
```bash
claude --plugin-dir plugins/kryos-iot-cognitum
```
...
- `prax-agentdb` — HNSW-indexed telemetry storage backend
- `kryos-intelligence` — SONA neural pattern learning
- `kryos-observability` — Telemetry correlation and tracing
