...
```bash
claude --plugin-dir plugins/kryos-observability
```
...
- `kryos-cost-tracker` -- Token usage metrics feed into cost attribution
- `kryos-iot-cognitum` -- Reuses Z-score anomaly detection for telemetry patterns
- `kryos-market-data` -- Data feed health and ingestion latency monitoring
