...
- **prax-agentdb**: Underlying storage for entities, relations, and causal edges via HNSW-indexed AgentDB
- **kryos-core**: Researcher agent uses pathfinder traversal for codebase exploration
- **kryos-ruvector**: HNSW indexing for fast semantic search across graph nodes
- **kryos-intelligence**: SONA neural patterns learn from graph traversal trajectories
