...
- **kryos-goals**: Use horizon tracking to place SPARC features within long-term planning horizons. Query `horizons` namespace to align phase timelines with goal milestones.
- **kryos-workflows**: SPARC phases can be codified as workflow templates. Use `mcp__claude-flow__workflow_create` to create reusable phase workflows.
- **kryos-ddd**: Architecture phase (Phase 3) directly leverages DDD bounded context patterns. Query `ddd-contexts` namespace for existing domain models.
