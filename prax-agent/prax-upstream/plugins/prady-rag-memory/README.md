...
```bash
claude --plugin-dir plugins/kryos-rag-memory
```
...
When `kryos-ruvector` is also loaded, rag-memory delegates to ruvector's backend for advanced features:
...
- `prax-agentdb` — Full AgentDB with 19 controllers and HNSW search
- `kryos-ruvector` — Advanced vector operations (FlashAttention-3, Graph RAG, hybrid search)
- `kryos-rvf` — Portable RVF memory format for cross-machine export/import
- `kryos-knowledge-graph` — Entity extraction and graph traversal over memory
