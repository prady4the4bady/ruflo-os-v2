...
- **kryos-intelligence**: Model routing optimization data feeds cost analysis (3-tier routing reduces cost 75%)
- **kryos-autopilot**: Budget-aware autopilot mode uses cost data to throttle agent spawns
- **kryos-observability**: Token usage metrics are collected via observability instrumentation
- **kryos-swarm**: Agent spawn/stop decisions informed by budget remaining
