...
```bash
claude --plugin-dir plugins/kryos-migrations
```
...
- `kryos-adr` -- Document schema change decisions as ADRs
- `kryos-ddd` -- Align migration boundaries with aggregate roots
- `kryos-observability` -- Track migration execution duration and failure rates
