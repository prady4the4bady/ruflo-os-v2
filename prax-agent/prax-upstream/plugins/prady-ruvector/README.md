...
Wraps the `ruvector` npm package as a Kryos plugin, providing vector embedding, semantic search, clustering, hyperbolic space reasoning, self-learning hooks, and Brain AGI diagnostics. ruvector's Rust backend delivers sub-millisecond queries and 52,000+ inserts/sec.
...
```bash
claude --plugin-dir plugins/kryos-ruvector
```
...
- `prax-agentdb` — HNSW storage backend in AgentDB
- `kryos-intelligence` — SONA pattern learning integration
- `kryos-knowledge-graph` — Graph RAG for multi-hop retrieval
- `kryos-rag-memory` — Simple semantic search via ruvector
