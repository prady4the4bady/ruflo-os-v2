...
```bash
claude --plugin-dir plugins/kryos-market-data
```
...
- `kryos-neural-trader` -- Consumes market patterns as strategy signals
- `kryos-ruvector` -- HNSW indexing engine for pattern similarity search
- `kryos-observability` -- Data feed health and ingestion latency dashboards
